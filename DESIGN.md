...
- **Responsive behavior:** the rail never wraps, including around 320px. It scrolls horizontally inside its own box and the focused or active tab scrolls into view. The Capability Lab outer shell owns a container query so its title/description and diagnostic badges collapse to one column even when the Settings modal viewport itself remains wide; CJK copy must retain a readable line width instead of collapsing to one character per line. The page and backend workspaces must not gain horizontal overflow; table shells remain the only other horizontal scroll owners.
