...
| 批次 | 编号 | 需求 | FlowText 对应 | 优先级 |
|---|---|---|---|---|
| A | R-A1 | 笔记内 `@` 唤起与默认热键 | `@` 呼出悬浮窗 | P0 |
| A | R-A2 | `#` 预设提示词菜单 | `#` 快速调出预设提示词 | P0 |
| A | R-A3 | 流式 diff 预览 | 流式续写 | P0 |
| A | R-A4 | 行内面板贴图（OCR / 手写 → LaTeX） | 截图转公式 | P0 |
| A | R-A5 | 多片段并行编辑 | 多片段并行续写/修改 | P1 |
| A | R-A6 | 全文修改模式 | 点击机器人图标改全篇 | P1 |
| A | R-A7 | 文件夹 / 多选上下文 + 拖拽 | 选择整个文件夹、内链嵌入 | P1 |
| B | R-B1 | 生成内容自动内链 | 自动建立指向参考笔记标题的内链 | P1 |
| B | R-B2 | 主题关联（上下文组） | 勾选关联主题作参考资料 | P1 |
| B | R-B3 | 编辑回退（单文件 / 整轮） | 单文件回退、一键回退全部 | P1 |
| B | R-B4 | Obsidian 原生工具 | 基于官方 CLI 的 Agent | P1 |
| B | R-B5 | 批量整理（归拢笔记 / 批量改属性） | 批量操作与整理 | P2 |
| C | R-C1 | 整库语义检索（整库感知） | 整库感知模式 | P2 |
| C | R-C2 | 文生图 | 文本生成图片 | P2 |
| C | R-C3 | Alt 一键补全（ghost text） | Cursor 风格自动补全 | P2 |
| C | R-C4 | PDF 读取 / 索引 / 内文交互 | 超大型 PDF 本地索引 | P2 |
| C | R-C5 | Canvas 生成与节点级 AI | Canvas 白板支持 | P2 |
| C | R-C6 | 外部接口（远程驱动） | DeepSeek Harness 接口 | P2 |
