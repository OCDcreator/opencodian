# Owner: core.storage
> 2026-09-21 (advantage-parity R-F3 质量修复)：`closeRound()` 成为 turn/batch 结束路径唯一的关闭缝，post baseline 冻结改为 best-effort（永不抛出，失败条目落 `postImageUnavailable`）且带 500ms 墙钟预算；修复"冻结写盘异常使轮次保持开放、持续误归属后续用户编辑"的缺陷。回归测试 `tests/unit/core/storage/EditRevertService.baselineClosure.test.ts`。
