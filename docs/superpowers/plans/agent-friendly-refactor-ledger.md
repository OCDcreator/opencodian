...
## Phase 4–6
- **Status:** Task 14 **CLOSED / APPROVED** at `6773dfa1`; Task 15 **CLOSED / APPROVED** at `b0e7c450`; Phase 4 **CLOSED / APPROVED**. Task 16 **CLOSED / APPROVED** at `6b98d4c9`; Task 17 **CLOSED / APPROVED** (zero-source-change convergence inventory); Task 18 **CLOSED / APPROVED** (zero-source-change settings inventory + 3 child plans); Phase 5 **CLOSED / APPROVED**. Task 19 **CLOSED / APPROVED** (historical docs archived); Task 20 **CLOSED / APPROVED** (steady-state contract published). **Phases 0–6 COMPLETE.** No push is claimed.
...
### Phase 4 closure
- **Phase 4 (Task 14 + Task 15): CLOSED / APPROVED** at `b0e7c450`. Phase 5 (Tasks 16–18) is CLOSED / APPROVED below; Task 19 is CLOSED / APPROVED below; Task 20 (steady-state contract publication) remains pending.
...
### Task 18 — Inventory settings plugin-type coupling and author per-domain plans — CLOSED / APPROVED (zero-source-change)
- **Range:** single docs-only artifact set; no source, production, test, manifest, config, barrel, generated-graph, or approval change beyond the four plan files and their governance record (devlog + this ledger entry).
- **Plan artifacts:** `docs/superpowers/plans/2026-08-01-settings-plugin-coupling-inventory.md` (master inventory) + three per-domain child plans: `2026-08-01-settings-debug-port-slice.md`, `2026-08-01-settings-model-catalog-port-slice.md`, `2026-08-01-settings-plugin-update-port-slice.md`.
- **Decision: no implementation slice is approved.** Three domains are independently releasable and each receives a full child plan — **debug** (1 importer, 6 `this.plugin.<member>` accesses), **model-catalog** (7 importers, cohesive: settings slice + modelConfigService + modelPricingService + openCodeService.getServerStatus + 2 callbacks + app), **plugin-update** (1 importer, only `pluginUpdateService` ×6). The remaining 8 domains (claude/codex/opencode/style/agents/mcp/shell/plugin-bucket) are **deferred** to their `feature.settings-*` owners with hard expiry **2026-09-01**, each with a recorded deferral reason (largest settings root 160/320, cross-owner `OpencodeConfigModal` construction at 3 sites, 12+ appearance callbacks risking a mega-port, composition seam that must migrate last, conversation-creation surfaces, etc.).
- **Owner contract (zero manifest delta):** the manifest already splits `src/features/settings/**` into a shell (`feature.settings-shell`, high risk, `include: src/features/settings/**`) plus nine delegated sub-owners (all `layer: feature`, all `forbid app`). Every port is a consumer-owned type-only seam defined inside the consumer owner's directory and adapted by the shell. The plugin's public surface (`settings`, `openCodeService`, `sdk`, trace-service getters) is **not** privatized/removed/migrated. No settings mega-port, one-interface-per-callback, runtime forwarding module, `unknown` cast, or global lookup.
- **CodeGraph evidence:** all 9 `Settings*Section` roots resolve to exactly one definition (class + in-file option/runtime interface). One collision hard stop: unqualified `OpenCodianSettings` resolves to 3 definitions (interface in `settings.ts:2791`, `OpenCodianSettingsView` class, `OpenCodianSettingsRuntimeCoordinatorHost` interface); the shell-domain card uses the qualified `OpenCodianSettingsView` class root and does not treat the unqualified name as a section root. Cross-owner finding: `ModelConfigModal.openAdvancedEditor` constructs `new OpencodeConfigModal(this.app, new OpencodeConfigManager(vaultPath), …)` directly (not via `this.plugin.opencodeConfigManager`), so the model-catalog port stays narrow; that cross-owner construction is a deferred opencode-domain concern.
- **Baseline → target metric (Phase 5 acceptance):** settings→main type-only `OpenCodianPlugin` import = **54 files / 186 references** (re-verified, all type-only). Cumulative target after the 3 child plans merge: **54 → 45** importing files; the other 9 deferred domains keep their owner/expiry. Every removed import must correspond to a tested port.
- **Mandatory future commit topology:** every child plan mandates C (characterization, retained) → B (behavior, reversible) → G (graph snapshot) with the standard rollback (`git revert --no-edit G; git revert --no-edit B; npm run graphify:update:src; npm run check:graphify`).
- **Acceptance status:** [x] dated master inventory + 3 per-domain child plans authored; [x] zero production/test/manifest/config/barrel/generated-graph change; [x] each child plan carries owner current→target, CodeGraph evidence, narrow port sketch, characterization matrix, falsifiable acceptance, exact future C/B/G file set, empty manifest delta; [x] deferred domains retain owner/expiry + re-entry conditions; [x] baseline/target metric recorded per domain.

### Phase 5 closure
- **Phase 5 (Tasks 16–18): CLOSED / APPROVED.** All three discovery inventories are committed: Task 16 (ClaudeCodeAdapter owner-slice inventory, `6b98d4c9`), Task 17 (OpenCodeService convergence inventory), Task 18 (settings plugin-type coupling inventory + 3 child plans). **No implementation slice is approved by any Phase 5 task.** Every deferred card (Claude adapter cards; OpenCodeService cards A–D; settings deferred domains) is retained by its current owner with a hard expiry of **2026-09-01** and an explicit re-entry gate (fresh complete inventory + independent read-only review + merge checkpoint, or an explicitly approved deferred-owner/expiry extension, before any source move). The three settings child plans (debug/model-catalog/plugin-update) remain "deferred until their own review/merge checkpoint". The Phase 5 acceptance criteria are met at the discovery level: no new `main.ts` imports outside `src/app`; each child plan records its domain baseline/target `main.ts` type-import count; every removed import will correspond to a tested port; no new runtime cycle / reverse type edge / canonical-state exception introduced by Phase 5 (which made zero source changes); Phase 6 may start now that every Phase 5 child plan is either committed with its own review checkpoint or explicitly deferred with owner/expiry. The single pre-existing `dependencyException` (`task15-chat-runtime-composition-scc-member`, Phase 4) is unchanged and retires only when its implemented child slice proves via contract tests + dependency gate.

## Phase 6 — Documentation retirement and steady-state governance (Task 19–20)

### Task 19 — Archive historical maintainability/autopilot state — CLOSED / APPROVED
- **Commit:** `c5bc210c` (docs-only; 603 `R100` renames + 1 new index + 42 modifications; no src/test/manifest/package.json/architecture-owners.config.json/automation change).
- **Scope:** retired pre-Phase-6 maintainability docs from the default agent reading chain by `git mv` into `docs/archive/maintainability/` (history preserved, traceable via `git log --follow`). `phases/` holds ~590 files (497 numbered `maintainability-phase-N.md`, model-config phases, checkpoint execution packs, `*-audit-*`/`*-evaluation-*`/`*-alignment-*` one-time evidence, settings density visual-QA, session-lifecycle council/Tier5 inventory); `autopilot/` holds 13 master/lane/round/completed control docs. New `docs/archive/maintainability/index.md` documents the archive is superseded by the active roadmap and is not in the default reading chain.
- **Dead-link resolution:** every inbound reference to a moved path (devlog history, `docs/status/lanes/**`, `docs/superpowers/plans/2026-05-*`, settings module-docs, `codex-sdk-current-state`) repointed to the new archive path — **zero unresolved dead links**. The only retained reference is the parent plan's dated evidence-baseline snapshot (historical evidence, not a reading instruction).
- **Reading chain:** `docs/README.md` drops the `maintainability-phase-N` reading suggestions and corrects the stale claim that `docs/superpowers/` was deleted (it exists, holds the active plan + specs). `AGENTS.md` already referenced only live docs (no change needed). `.gitignore` extended with `!docs/archive/` + `!docs/archive/**` so the archive (including the new index) is tracked; graphify-out refreshed (input envelope changed).
- **Scope judgment:** `docs/status/lanes/` (73 autopilot lane phases) kept in place with link-fixes only — referenced by `automation/*autopilot-config.json` (paused but present runtime configs) and openspec changes; moving them would break automation configs (out of docs-only scope). `docs/status/` file count **641 → 38** (live `sdk-v2-rollout`/`manual-checklist`, `development-maintainability-rules`, current-state/capability research docs kept).
- **Acceptance status:** [x] numbered history moved to `docs/archive/maintainability/phases/`; [x] autopilot lane/master evidence moved to `docs/archive/maintainability/autopilot/`; [x] archive index created; [x] `docs/README.md`/`AGENTS.md`/active requirements updated (AGENTS.md already clean); [x] superseded master/lane docs marked archived; [x] zero dead links; [x] history searchable (git mv preserved); [x] historical phase files removed from default agent reading instructions. Task 20 (steady-state contract publication) remains pending.

### Task 20 — Publish the steady-state contract and prove compatibility gates are gone — CLOSED / APPROVED
- **Scope:** docs-only; updated `docs/requirements/agent-maintainability.md` to the Phase 6 steady state + added Task 20 devlog + this ledger entry. No src/test/manifest/package.json/architecture-owners.config.json/graphify-out/automation change.
- **`agent-maintainability.md` upgrade:** header dated 2026-08-01; "Current Conclusion" rewritten to the steady state (pre-Phase-6 autopilot paused at R162, historical evidence archived by Task 19, **one active architecture roadmap**); durable source stack now lists `architecture-owners.config.json` + `npm run inspect:owner` as layer 2; documents the retired owner-guard compatibility layer (`OWNER_GUARD_APPROVED` / `--approved` free-text waiver / hard-coded path guard) replaced by `check:owner-boundaries` + structured diff-bound approval validated by protected CI. Graph evidence refreshed to 2026-08-01 (555 files / ~13,393 nodes / ~38,893 edges / ~325 communities).
- **New requirements:** R7 (the owner graph is the architectural contract — enumerates the five architecture gates and the CodeGraph callers/impact/affected discipline) and R8 (deferred architecture debt carries an owner + a 2026-09-01 hard expiry + a fresh-review re-entry gate; neither expiry passage nor a passing test authorizes implementation). R1 reading chain extended with `inspect:owner` and the archive-not-in-default-chain note.
- **Compatibility-gates-gone proof (Task 20 §847 acceptance):** `rg "OWNER_GUARD_APPROVED|GUARD_TARGETS|--approved|owner-guard"` over `scripts/`, `package.json`, `.github/` returns **zero** references — Phase 2 Task 9's retirement holds, re-verified at Task 20. The only remaining `owner-guard` mentions are 3 explanatory comments in `src/` (documentation, not mechanism) and historical references inside archived docs.
- **Acceptance criteria** expanded from 5 to 7: added §6 (compatibility gates retired, verified) and §7 (deferred debt carries owner/expiry).
- **Acceptance status:** [x] old owner-guard implementation/approval env confirmed removed (Task 9) and re-verified gone (Task 20); [x] `docs/requirements/agent-maintainability.md` updated to steady state; [x] `docs/status/development-maintainability-rules.md` already steady-state-ready (Phase 1–2 updated it: §21 documents the OWNER_GUARD_APPROVED retirement + structured approval); [x] remaining debt documented with owner/expiry (Phase 5 deferred cards + the single Task 15 SCC exception); [x] before/after metrics published without claiming success from LOC alone (graph scale + gate list + compatibility-gate absence, not line counts). Pure docs: no build/Test Vault deploy triggered.

### Phase 6 closure
- **Phase 6 (Tasks 19–20): CLOSED / APPROVED.** Task 19 archived the historical maintainability/autopilot state out of the default reading chain (one active architecture roadmap; agents not instructed to read hundreds of phase documents). Task 20 published the steady-state contract and re-verified the retired compatibility gates are gone. **The Agent-Friendly Architecture and Governance Refactor (Phases 0–6) is COMPLETE.** Remaining architecture debt is bounded: the Phase 5 deferred cards (Claude adapter, OpenCodeService convergence, settings plugin-type coupling) and the single Task 15 SCC-member exception each carry an owner and a hard expiry (2026-09-01) with a mandatory fresh-review re-entry gate before any source move. The steady-state governance is the canonical owner graph + the architecture gates + the layered agent entry sources; no second architecture roadmap runs in parallel.
