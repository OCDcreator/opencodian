...
集中维护 MCP 工具摘要的动作词、类别字段、通用回退字段和字段展示类型，并导出 `getMcpToolSummary(name, input)` 作为统一入口。`ToolCallRenderer` 现在直接复用这个 helper：先用工具名动作词判断类别，再按对应字段顺序从顶层 `input` 中提取摘要。
...
- 维护 MCP 动作词 → 字段优先级映射
- 规范路径类 / 文本类 / 参数类字段的展示格式
- 提供 `getMcpToolSummary()`，把类别命中、通用字段回退与最终标量回退封装到同一模块
