...
| `revertLastBatch()` / `hasLastBatch()` / `getLastBatchId()` | 一键回退最近一次批量（R-B3 `revertAll`，合成会话 id `batch-organize-…`），随后清理本批次创建且现已为空的目录；返回 `BatchRevertResult`（文件回退结果 + `leftoverFolders` 未能移除的残留目录） |
...
- **目标目录（R-B5-D1/D2b）**：`FileManager.renameFile` 不创建父目录——执行在快照门之后、首个写操作之前逐一确保目标目录存在（`vault.createFolder`，按父先子后的确定性排序幂等创建）；任一目录创建失败即 fail-closed（回滚刚创建的目录、返回 `folder-unavailable`，UI 显式提示，零写入）。预览如实列出"将新建目录"；回退后移除本批次创建且现已为空的目录（最深优先，`adapter.list` 空判定，**绝不删除非空目录/用户内容**），库形态恢复到操作前；全部操作失败（changed=0）时立即清理刚建目录。R-B5-D2b：删除必须走 **vault API**——`vault.getAbstractFileByPath` 解析为 `TFolder` 后 `vault.delete(folder, true)`（永久删除，非回收站：目录是本批次刚创建且已验证为空，R-B5 承诺恢复操作前形态，回收站条目反而是新状态）。Adapter API 两条路在真机上都不通：`adapter.remove` 仅适用于文件（对目录抛错），`adapter.rmdir` 对目录同样抛 `EISDIR`（Obsidian 1.13.7 实机探针，空目录、中英文目录名均复现；`obsidian.d.ts` 的类型拆分看不出这一点）——`7deadbab` 改走 `rmdir` 正因此在线上失效而单测全绿（替身把 `rmdir` 建模成可用）；`force: true` 与实测探针一致，避免隐藏未索引文件导致误拒，内容安全由紧邻其前的空判定承担。未能移除的目录（用户内容保留、vault 索引中无法解析或删除失败）经 `BatchRevertResult.leftoverFolders` / execute 出口的 `leftoverFolders` 返回，UI 以 `batchOrganize.notice.revertLeftoverFolders` 显式提示，不再静默。目录存在性不参与 stale-plan 签名（建目录幂等，用户中途自建的目标目录会被直接采用）。
