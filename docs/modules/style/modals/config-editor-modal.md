...
- MCP 设置：`.opencodian-mcp-*`（overview toolbar + metric cards、server status rows、status badges、grouped add-server form）。
...
- `opencodian-mcp-server-row-main` 默认是三列对齐，但窄屏会退化成单列；如果修改行级 grid，记得同时检查状态 badge 和按钮在移动宽度下不会重新挤压换行得太难看。
