...
- detected formatter 表格搜索和 builtin formatter / LSP 长列表都使用 `.opencodian-builtin-list-search*` 自定义搜索控件：inline toolbar、monospace 计数 chip、透明清除按钮、row-token 空态，以及 Obsidian-native 的绝对定位建议浮层；不使用原生 datalist/select，也不引入独立卡片层级。builtin 列表的内部滚动容器 `.opencodian-formatter-builtin-scroll` 自带 top padding / scroll padding，避免滚动时 row 被 sticky 搜索框贴住；右侧同时预留 scrollbar gutter，让 sticky 搜索框和滚动列表的视觉宽度保持一致。过滤时 TS 会给第一个和最后一个可见 builtin row 标记 `.is-first-visible` / `.is-last-visible`，CSS 用它们补足列表和 section 底部之间的 spacing；无匹配时 `.opencodian-builtin-list-search-empty` 负责保留底部 breathing room，避免隐藏行破坏 `:last-of-type` 间距。
