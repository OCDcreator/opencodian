...
- 按钮语义：`.mod-cta`（允许）继承宿主的 accent/on-accent 配对（理由见 `batch-organize-modal.md`：压暗 accent 在当前主题下实测更差，2.87:1 → 2.2:1，因为标签是近黑而非浅色）；拒绝/破坏性路径使用原生 `.mod-warning`（rose）并自定深浅配色，与仓库既有的行内权限拒绝词汇一致。
