...
- `.opencodian-icon-cache-modal-summary` : 顶部信息汇总卡片，带有细微的阴影。
- `.opencodian-icon-cache-quick-jump` : 提供商快速跳转导航区，采用 `sticky` 粘性定位与毛玻璃背景。
...
- **Quick Jump 粘性导航**: 滑动时吸顶，背景变为半透明 `backdrop-filter: blur(8px)`，防止与下层内容视觉冲突。
- **卡片 Hover 态**: 
