...
- `.opencodian-server-status-badge`：聊天 header 的 server/backend 状态按钮。默认为 28px 紧凑 shadcn-style badge，内部的 `.opencodian-server-status-icon` 用 active backend identity 表达后端；OpenCode 使用与标题 logo 同源的 inline brandmark，Claude Code 复用设置页后端导航的 LobeHub `claudecode` 彩色 light/dark 图标，其余非 OpenCode backend 使用 provider SVG mask。`.opencodian-header-status-group::before` 保留 28px 的稳定 flex 占位，而 badge 本身以该占位的右缘为锚点绝对定位；`.opencodian-server-status-badge.opencodian-tooltip-trigger` 明确保留该绝对定位，避免共享 tooltip trigger 的相对定位覆盖它。hover / focus-visible 时它在 header 内向左展开、使用现有 local hover overlay 层级（`z-index: 8`）覆盖左侧相邻内容，不会把 LSP 或右侧 action 挤开。`.opencodian-server-status-state` 是 badge 级右下角 ring/dot，不参与图标居中；`.opencodian-server-status-text` 在相同按钮内展开到 `min(220px, 48vw)` 的有界空间，不使用省略号，鼠标离开后自动收回。`.is-disabled` 是后端全部禁用时的灰色状态，与 `.is-checking`/`.is-starting`/`.is-running`/`.is-external`/`.is-offline` 并列。
