...
- `.opencodian-server-status-badge`：聊天 header 的 server/backend 状态按钮。默认为 28px 紧凑 shadcn-style badge，内部的 `.opencodian-server-status-icon` 用 active backend identity 表达后端；所有 backend 复用设置页后端导航的 LobeHub light/dark 图标。`.opencodian-header-status-group::before` 保留 28px 的稳定 flex 占位，而 badge 本身以该占位的右缘为锚点绝对定位；`.opencodian-server-status-badge.opencodian-tooltip-trigger` 明确保留该绝对定位，避免共享 tooltip trigger 的相对定位覆盖它。hover / focus-visible 时它在 header 内向左展开、使用现有 local hover overlay 层级（`z-index: 8`）覆盖左侧相邻内容，不会把 LSP 或右侧 action 挤开。`.opencodian-server-status-state` 是 badge 级右下角 ring/dot，不参与图标居中；`.opencodian-server-status-text` 用与 presenter 相同的 46px inline chrome 预算（图标、gap、边框和 ring 安全区）扣除可用文字宽度，因此测量到的短标签可以完整显示，同时仍受 220px / 48vw 上限约束且不使用省略号，鼠标离开后自动收回。`.is-disabled` 是后端全部禁用时的灰色状态，与 `.is-checking`/`.is-starting`/`.is-running`/`.is-external`/`.is-offline` 并列。
- `.opencodian-theme-background-*`：主题背景图层、遮罩、叠加高光。
- 关键变量组：`--opencodian-glass-*`、`--opencodian-composer-*`、`--opencodian-status-*`、`--opencodian-shadow-*`。其中 `--opencodian-status-{warning,success,error,info}`（含 `-subtle` / `-border` 变体）定义在 `body` 作用域而非 `:root`：Obsidian 只在 body 上定义 `--text-success` / `--text-error` / `--text-warning` / `--color-*`，`:root` 声明引用它们会得到 guaranteed-invalid 并随继承传播失效。`info` 族基于 `--color-blue`（回退 `--interactive-accent`），为 capability-lab readback/pending 等语义芯片提供蓝色信息态。原始状态色相在小号 chip 文本上达不到 4.5:1，`.theme-dark` / `.theme-light` 两个覆盖块把文本级 token 分别向 `--text-normal` 调亮（dark 的 error/info）或调暗（light 全部四个），`-subtle` / `-border` 由基础 token 自动派生。
