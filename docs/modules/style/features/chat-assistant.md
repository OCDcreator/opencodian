...
2026-09-26：聊天内联卡片沿用同一套 Obsidian card 变量（背景、边框、圆角、阴影），后台任务、权限与问题卡的状态以图标、文字和操作层级表达。样式需与真实聊天 DOM 及对应渲染模块一同验收。
...
- Turn Change Record 紧凑卡片：`.opencodian-chat-notice-card.is-turn-diff` 命名空间下的 `.opencodian-turn-diff-*`——根卡片显式移除 generic info notice 继承来的边框与阴影，避免 hover 时与 assistant message 背景叠成第二层白框；header 数量徽标、单行 `nowrap` 文件行（`min-height: 30px`、4px gap、`hidden` 属性显隐覆盖 flex）、可收缩弱化父目录 + 优先可见的文件名、`+N`/`−N` 统计徽标（`--opencodian-status-success(-subtle)` / `--opencodian-status-error(-subtle)` 弱背景、`font-variant-numeric: tabular-nums`）、`added`/`deleted` 状态标签（`.status-renamed` 仅预留视觉映射，无数据）、DOM-local 展开/收起 toggle；row selector 使用足够 specificity 显式清除 Obsidian button 的常态背景/阴影，hover 仅轻背景，`:focus-visible` 用 accent outline，无内部滚动、无第二层卡片边框、无阴影。
...
- 输入位置中，直接区域顺序为 Todo → 问题卡 → 会话标签 → Composer；`.opencodian-input-area` 对隐藏的 `.opencodian-session-todo-dock`／`.opencodian-question-dock` 父 slot 使用 `:has()` 退出 flex，对空的 input tab slot 使用 `:empty` 退出 flex。因而隐藏 dock 或关闭标签不会累计空 slot 间距；可见标签与 Composer 仅保留 input area 的统一 8px gap，Todo／问题卡实际可见时仍保留在标签上方，Header／标题下方的标签挂载位置不变。
...
- Composer 的 Agent / permission / model / runtime-overflow 菜单打开时，`.opencodian-input-area` 临时升至 `z-index: 230`，必须高于外置会话标签宿主的 `z-index: 220`；菜单关闭后保留默认 `40`，避免静态输入区持续压住标签。
