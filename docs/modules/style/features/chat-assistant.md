...
- `.opencodian-runtime-overflow` / `.opencodian-runtime-overflow-trigger` / `.opencodian-runtime-overflow-panel`：runtime rail 的低优先级 readback 状态入口。trigger 使用 `order: 999` 与 `margin-left: auto` 固定为最后一个视觉 affordance，回到输入框右下/最右端；default/独立按钮模式下 trigger 是与 model / permission 一致的独立 pill；`.opencodian-composer-shell--action-buttons-etched` 下变为透明刻入式按钮。panel 承载 sandbox、additional directories 和 Codex runtime defaults 等只读徽章，避免主输入区塞入长状态 chip。
