...
- 输入工具栏现在为 Agent / permission / model / effort selector 统一保留 flex slot，并通过 `--opencodian-input-toolbar-control-gap` / `--opencodian-input-toolbar-control-height` 统一控制横向节奏与控件高度；context usage ring 也跟随该高度，并在 `action-buttons-etched` 下切换成刻入式透明状态。Agent 下拉框本体样式在 `components/agent-selector.css`。
