...
- `.opencodian-inline-edit-inputrow` / `-inputfield` / `-inputlead` / `-field`：输入行——`inputfield` 是**字段框体**（填充底 + 发丝内环 + `0 10px` 内边距 + 聚焦态强调色内环），品牌标记（`opencodian-app-icon`）与输入框都在框内；`field` 是裸 `<input>`，其全部盒属性（border/background/box-shadow/padding/height/outline/font-size）都用 `!important` 钉死

  **为什么字段框体归我们所有**：宿主主题与用户 CSS 片段会按自己的规则给裸 `input[type=text]` 加边框和内边距，实测出现过"有边框、零内边距、placeholder 贴着框线"的线上表现。这个浮层必须在任意主题下长相一致，所以框体由 `.opencodian-inline-edit-inputfield` 承担，输入框本身被压成透明无边框；`.obsidian-debug/style-repro` 的 mock 现在也模拟宿主表单样式，并由 `verify-dark.cjs` 断言"输入框无边框/无内边距、框体有填充与内边距"——不要为了"看起来更简单"把这些 `!important` 删掉
