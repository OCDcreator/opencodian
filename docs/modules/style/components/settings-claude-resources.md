...
Claude 资源管理设置面板的样式，与 `settings-codex-resources.css` 对称、同属 Settings Extension Surface 词汇。**层级统一**：资源页无外层 section 大卡片；每个资源类型（commands / skills / agents）是独立的 `.opencodian-resource-group-card`（与 Codex 资源页共享同一 card 类）。为 `.opencodian-claude-resource-*` 元素提供：扁平紧凑组头（h4 左侧、新建按钮右侧，且以 8px 最小间距保持垂直居中）、ScrollArea 有界列表、结构化 row-card（名称 + tonal scope badge + ghost 操作 / 描述 / 等宽 11px 路径）、整框 tonal 的 user-source 提示（无左侧色条），以及创建/编辑弹窗（等宽 textarea + 右对齐 action 行）。独有 `.is-global-disabled`（warning tonal，无斜体）表达「已发现，未启用」。
