...
- `.opencodian-debug-workbench` / `.opencodian-debug-status-strip` / `.opencodian-debug-channel-list` / `.opencodian-debug-log-preview`：Claude Code 调试工作台布局。它参考格式化器子标签的 object panel、row list 和 pill 元信息节奏，使用紧凑 summary strip、通道 row 设置项和 monospace 日志预览，不再把 Claude Debug 渲染成单个空 toggle，也不使用 dashboard 式指标卡或嵌套重卡片。
