...
- `.opencodian-sandbox-config-badge*`：permission trigger 旁的 Claude Code sandbox 配置徽章，用于显示 expanded sandbox 摘要；作为 readback-style detail chip，使用 muted pill、ellipsis 与小图标，不展开成长说明。
...
- `.opencodian-codex-runtime-defaults-badge*`：permission trigger 旁的 Codex runtime defaults 徽章，仅在网络访问启用、网页搜索未禁用或额外目录非空时渲染，使用紧凑 runtime-chip 几何与 ellipsis 截断。
