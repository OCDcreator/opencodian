...
这个入口不暴露 `commandScopedAgent.ts`、`modelConfig.ts`、`slashCommandCatalog.ts` 里的辅助函数或类型，因此需要这些细粒度工具的调用方仍然要走深路径导入。
...
barrel 没有转发 `commandScopedAgent.ts`、`modelConfig.ts`、`slashCommandCatalog.ts` 中的辅助导出。仓库中需要这些内容的模块会直接引用：
...
- `commandScopedAgent.ts` 是目录内私有 helper；即使职责变更，也应优先保持深路径导入而不是直接扩张 barrel。
