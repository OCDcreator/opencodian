...
7. 同步 `.opencode` 权限配置
8. 当设置已经回到 `enabledBackends` 包含 `opencode`、`activeBackend=opencode` 且 `server.mode=local`、`autoStart=true` 时，调度一次 deferred runtime warmup，让从 disabled/remote/offline 恢复到本地托管时能自动拉起服务，而不是等到下次 reload 或会话 bootstrap；如果 Codex / Claude Code 等非 OpenCode 后端正处于 active 状态，则不调度 OpenCode health snapshot，避免无关 warning 噪音。
