...
- 通过 `AgentSessionCapability` 委托 `createSession()`、`deleteSession()`、`updateSessionTitle()`，并以 `listSessions()` / `getSession()` / `getSessionMessages()` 保留 OpenCode session directory 与消息历史访问能力；`cancelStream(sessionId)` 保持现有取消流行为
