...
- 只暴露 `query({ prompt, options })` 这一条 OpenCodian Phase 1 所需 facade
...
- 不在本模块中保存 session 状态或做 OpenCodian stream normalizing；这些责任属于 `ClaudeCodeAdapter` 与 `ClaudeCodeStreamNormalizer`
