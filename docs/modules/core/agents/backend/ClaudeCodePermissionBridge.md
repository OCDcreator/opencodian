...
- 通过 `createLogger('ClaudeCodePermissionBridge', { moduleKey: 'claudeCode' })` 记录 canUseTool request/decision 和 AskUserQuestion request/decision/cancel/error 摘要；日志字段限制为 `toolName`、`toolUseID`、`patternCount`、`questionCount`、`behavior`、`reply`、`updatedPermissionsCount`，不记录用户回答、完整 input、问题文本或额外上下文字段

## 维护约束

- 不在这里渲染 UI；host callback 由后续 Claude adapter/runtime 注入。
- permission bridge 的 `debug` / `info` 日志必须继续受全局 `enableDebugLogging` 和 `claudeCode` module 控制；不要绕过 shared logger 或新增独立 buffer。
