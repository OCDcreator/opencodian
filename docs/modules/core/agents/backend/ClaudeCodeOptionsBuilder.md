...
- 只在 runtime 明确注入时透传 `hooks`、`sessionStore` / `sessionStoreFlush`、`outputFormat`、`plugins`、`skills`（包含 SDK 的 `'all'` skills sentinel）、`agent` 和 `agents`，为后续 Claude Code authoring、structured output、agent definitions 和 JSONL mirror/import 诊断保留官方 SDK 通道；这些字段不来自用户设置，也不等同于稳定 UI 已完成
