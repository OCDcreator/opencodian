...
- 只在 `settings.askUserQuestionPreviewFormat` 为 `'markdown' | 'html'` 时写入 `toolConfig: { askUserQuestion: { previewFormat } }` 到 SDK options。分类为 readback：settings→SDK option wiring 已证明；插件 question UI（inline card 与 dock）会保留并安全显示 SDK 提供的预览文本，但实际预览是否到达取决于 SDK 版本和模型行为，无法从插件层独立验证。HTML 预览以纯文本显示，不渲染富 HTML；预览仅在选项获得焦点或悬停时显示，不常驻在所有选项下方。默认空字符串 `''` 表示不请求预览（SDK 默认）。UI 位于 Tools 标签页下拉选择框，仅在下一次查询或重启会话时生效。这是 Claude-only 设置
