...
- 只在 `sandbox.enabled === true` 时写入 `sandbox` 到 SDK options；除 `enabled`、`autoAllowBashIfSandboxed` 外（`failIfUnavailable` 现在始终显式双向传递：省略时 SDK 对 `enabled: true` 缺省 fail-closed，会把插件设置"沙箱不可用时允许无沙箱运行"静默丢弃——CLI ≥ 2.1.2xx 在 OS 沙箱未激活的机器上会直接报错），还接线 `excludedCommands`、`allowUnsandboxedCommands`、`filesystem`（allowWrite/denyWrite/denyRead）、`network`（allowedDomains/deniedDomains）、`enableWeakerNestedSandbox`、`enableWeakerNetworkIsolation` 和 `ripgrep`（command/args）。分类为 readback：option wiring 已证明，OS 级 sandbox 强制执行无法从插件层独立验证
...
- 只在 `settings.outputStyle` 非空且 trimmed 后非空时，将 `outputStyle` 写入 SDK options 的 inline `settings` 对象：`options.settings = { outputStyle: trimmed }`。不创建 top-level `options.outputStyle`。Builder 本身只证明 settings→SDK option wiring；整体 Output Style 能力已通过 `runOutputStyleLiveProbe()` 获得 fresh diagnostic query 行为证明：临时 `.claude/output-styles/<style>.md` custom style 中的 nonce 未出现在用户 prompt 中，但可被模型响应回忆。诚实边界：这证明 custom output style file 能影响新查询，不证明活跃会话 live mutation，也不证明当前保存的 style name 一定有效。根据 SDK 文档，output style 会在会话启动时读取；修改会在 `/clear` 或新的 Claude Code 会话后生效，已有活动或恢复会话可能继续使用原先的系统提示词。UI 位于 Model & Thinking 标签页，为文本输入
