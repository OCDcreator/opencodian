...
- 导出 Claude Code adapter、runtime catalog readback 类型、project skill discovery helper、model catalog projection、SDK loader、options builder、MCP config adapter、process resolver、stream normalizer 与 permission bridge；`claude-code` 已可在设置中显式启用，默认仍保持 OpenCode
