...
- 全局 skills：`~/.agents/skills/<name>/SKILL.md`（只读）
- 全局 agents：`~/.codex/agents/<name>.toml`（只读）

全局资源严格只读——OpenCodian 绝不写入 `~/.claude`、`~/.agents` 或 `~/.codex`（产品约束）。只有 vault 内的项目根可写，且每次写入都经过校验（必需字段、名字安全、重复检测、路径穿越保护）并以原子方式（临时文件 + rename）完成，绝不残留半成品。Codex legacy `~/.codex/prompts` 有意不被发现或展示。
...
| `discoverCodexGlobalSkills(homePath)` | 扫描全局 `~/.agents/skills`，返回只读 skill 列表 |
