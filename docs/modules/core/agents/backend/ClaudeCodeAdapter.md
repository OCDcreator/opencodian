...
- `runDiagnosticPrompt()` 属于 Capability Lab 的 runtime-only side channel：它可以选择 `persistSession: false` 做无痕诊断，也可以附带 `sessionStore` 做 mirror/import proof，或通过 `resumeSessionId` 验证 SDK `options.resume` 是否能恢复既有 Claude session；无论哪种都不能修改稳定 chat runtime owner、settings 持久化或 user-facing capability gating。
