...
- 通过 `claudeCode` debug module 记录 SDK message summary 与产出 chunk summary，覆盖 text、thinking、tool_use、tool_result、backend_event、usage、error、message_metadata；日志跳过无 chunk 消息，并用 fingerprint 节流相同形状的高频 delta summary，只保留 type/subtype/session/message id、content length、metadata keys 和 counts，不记录文本全文、tool input 全文或 message metadata 的 model id

## 维护约束

- 这是 SDK stream 形状的本地兼容边界；引入 `@anthropic-ai/claude-agent-sdk` 后再把输入类型收窄到官方类型。
- normalizer 的摘要日志必须只使用 shared logger 的 `debug`/`info` 可选日志路径，继续受 `enableDebugLogging` 和 `claudeCode` module 控制。
