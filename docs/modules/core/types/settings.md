...
> 2026-07-30: `ClaudeCodeBackendSettings.sessionTrace` adds Claude Code trace settings: default-on capture, `off|standard|full` console preset, five channels from `CLAUDE_TRACE_CHANNEL_IDS`, and an optional storage directory.
...
| `normalizeClaudeCodeBackendSettings(value)` | 归一化 Claude Code executable、setting sources、permission/thinking/effort、additional directories、model、allowedTools/disallowedTools、maxTurns/maxBudgetUsd、env、file checkpoint、hook event、subagent transcript/progress、debug channel 与 `sessionTrace` 字段 |
...
| `modelPricingOverrides` | `ModelPricingOverride[]` | `[]` | 本地 provider/可选 endpoint/model 单价覆盖；仅用于无 backend 实际成本时的 API 等价估算 |
