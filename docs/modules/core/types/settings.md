...
`outputStyle` 为 readback only：通过 `buildClaudeCodeOptions` 传入 SDK `settings.outputStyle` 选项（值为 `string`），请求 Claude Code 以指定输出样式修改系统提示词。官方内置样式包括 `Default`、`Proactive`、`Explanatory`、`Learning`；用户也可以在 `.claude/output-styles` 或 `~/.claude/output-styles` 中创建自定义样式 markdown 文件。分类为 readback：SDK option wiring 已证明（`settings.outputStyle` 作为 inline settings 对象的一部分传递给 CLI 子进程），实际的系统提示词修改行为无法从插件层独立验证——没有结构化信号暴露当前系统提示词文本。根据 SDK 文档，output style 是系统提示词的一部分，会在会话启动时读取；修改会在 `/clear` 或新的 Claude Code 会话后生效，已有活动或恢复会话可能继续使用原先的系统提示词。默认空字符串 `''`。UI 位于 Model & Thinking 标签页，为文本输入。
