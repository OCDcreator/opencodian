...
| `normalize*` 系列 | function | 归一化函数（约 20+ 个），包含 Claude Code 专属的 `normalizeClaudeCodeStringArray`、`normalizeClaudeCodeNullablePositiveInt`、`normalizeClaudeCodeNullablePositiveNumber`、`normalizeClaudeCodeEnv` 和 `normalizeClaudeCodeDebugChannelSettings` |
...
| `OpencodeLspEntryConfig` / `OpencodeLspConfig` / `OpencodeLspStatus` | type | language server 条目、项目级 LSP 配置与运行时 LSP 状态 |
