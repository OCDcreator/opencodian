...
- `getSpawnEnv()` 负责清理污染 `OPENCODE_*` 覆盖项（通过 `LOCAL_SERVER_SANITIZED_ENV_KEYS` 循环），并按 `pluginIsolationMode` / auth 组装运行时环境。同时注入 `OPENCODE_EXPERIMENTAL_EVENT_SYSTEM=true` 以启用 `session.next.*` 实验性事件系统（对 1.15+ 安全，旧版本忽略该标志）。`modelSourceMode` 不再写 `OPENCODE_DISABLE_PROJECT_CONFIG`，避免只想改变模型来源时连带隐藏项目 skills/tools。
