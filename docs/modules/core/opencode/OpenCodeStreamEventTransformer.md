...
- 对 `session.next.tool.called`、`session.next.tool.success`、`session.next.tool.failed` 做增量 tool chunk 映射：`called` 用 `callID` 产出 `tool_use`，tool 名优先读 `tool` 并兼容 `name`，`success` / `failed` 用 `${callID}_result` 产出 `tool_result`，且不产出 canonical mutations，因为 v2 tool 事件没有可靠的 messageID / partID
- `session.next.tool.*` 与既有 `message.part.updated` tool 路径共享 `processedToolIds` / `toolInputSnapshots` 去重；如果同一个 `callID` 已经由另一条路径处理，后到事件只记录安全 debug metadata，不重复产出 tool_use / tool_result。若 v2 `callID` 与 part `callID` 不同，两条路径会各自独立渲染
- 对已知 `session.next.*` 实验性事件做 typed observe-only 处理：`session.next.agent.switched`、`session.next.prompted`、`session.next.step.started`、`session.next.text.started`、`session.next.text.ended`、`session.next.reasoning.started`、`session.next.reasoning.ended`、`session.next.tool.input.started`、`session.next.tool.input.ended`
- `session.next.step.ended` 会在 `tokens.input` 为有限数字时额外产出 `usage` chunk，并只累加有限的 `tokens.output` / `tokens.reasoning`，让 UI usage 显示包含模型生成的 reasoning token 且避免 NaN / Infinity 污染 usage 状态
