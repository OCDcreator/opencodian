...
- 对 `session.error` / `session.idle` 返回 stop 信号。`session.error` 现在通过 `classifySdkError()` 对错误分类，将 `errorClass` 记入 debug 日志并附加到 error chunk；`session.next.*` 实验性事件通过 `logStreamingDebug` 记录白名单字段（eventType、messageID、partID、toolID），不记录完整 properties 以避免泄露消息内容
