...
- 对 `session.next.tool.called`、`session.next.tool.success`、`session.next.tool.failed` 做增量 tool chunk 映射：`called` 用 `callID` 产出 `tool_use`，`success` / `failed` 用 `${callID}_result` 产出 `tool_result`，且不产出 canonical mutations，因为 v2 tool 事件没有可靠的 messageID / partID
