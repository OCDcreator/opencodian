...
- `SessionSyncEventUpdate`: reducer-ready 的 session sync payload，直接携带 canonical message / part 数据；`session.diff` 变体也会尽量附带已归一化的 diff entries，`session.compacted` 变体则作为“需要立即 refresh 当前 session”的轻量信号。
