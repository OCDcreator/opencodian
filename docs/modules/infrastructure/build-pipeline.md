...
GitHub 与 Gitea 均使用 Node 24 runtime 的 `actions/upload-artifact@v7`。Gitea 1.26.4 原生实现 ArtifactService v4 协议，且其集成测试覆盖 version 7 的分块上传、Finalize 与 SHA-256；因此无需再固定 legacy v3。上游 `@actions/artifact` 会仅凭 `GITHUB_SERVER_URL` 的第三方 hostname 抛出 `GHESNotSupportedError`，所以 Gitea 上传步骤把该变量局部覆盖为 action 明确放行的 `https://gitea.localhost`；真实上传地址和授权仍由 runner 注入的 `ACTIONS_RESULTS_URL` / `ACTIONS_RUNTIME_TOKEN` 指向 Gitea，不改变 checkout、API 或其他步骤的实例地址。打包内容由共享仓库脚本在上传前 fail-closed 校验。两端 workflow 都把 `OPENCODIAN_BUILD_ID` 固定为 `ci-<commit SHA>`，避免各 runner 的构建时间和时区进入 `main.js`，从而使同一 commit 的三件套可逐文件复现。
