...
GitHub 使用 Node 24 runtime 的 `actions/upload-artifact@v7`；Gitea 使用完整 URL `https://gitea.com/actions/gitea-upload-artifact@v7`。Gitea 1.26.4 原生实现 ArtifactService v4 协议，且其集成测试覆盖 version 7 的分块上传、Finalize 与 SHA-256。不能在 Gitea workflow 中直接复用 GitHub 上游 action：上游 `@actions/artifact` 会仅凭第三方 `GITHUB_SERVER_URL` 抛出 `GHESNotSupportedError`，而 runner 注入的标准环境会覆盖 step 级 hostname 伪装。Gitea 官方公共 `actions` 组织提供的专用 v7 action 移除了该 guard，使用 runner 注入的 `ACTIONS_RESULTS_URL` / `ACTIONS_RUNTIME_TOKEN` 对接 Gitea ArtifactService，因此无需 legacy v3 或环境变量伪装。打包内容由共享仓库脚本在上传前 fail-closed 校验。两端 workflow 都把 `OPENCODIAN_BUILD_ID` 固定为 `ci-<commit SHA>`，避免各 runner 的构建时间和时区进入 `main.js`，从而使同一 commit 的三件套可逐文件复现。
