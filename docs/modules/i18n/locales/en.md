...
2026-04-26 的 MCP settings UI 收口又补了一组 `settings.server.mcp.add.group.*` 和 `settings.server.mcp.add.type` 文案，用于把新增服务器表单重组为 `Basics` / `Connection` / `OAuth` 分组卡片，并把原先直接用标题占位的类型切换改成独立 `Type` 字段标签。
