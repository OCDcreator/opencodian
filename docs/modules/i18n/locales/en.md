...
> 2026-09-18 (R-B3): R-B3 strings: `settings.conversation.editRevertEnabled.*` / `editRevertSnapshotLimitMb.*` plus the sidebar keys `editRevert.sectionTitle`, `editRevert.revertibleShort`, `editRevert.revert`, `editRevert.revertAll`, `editRevert.restore`, `editRevert.excludedOversize`, `editRevert.excludedNoPreimage`, `editRevert.roundOpenHint`, `editRevert.degradedHint` and `editRevert.notice.{reverted,restored,revertAll,revertAllSkipped,failed}`.
...
2026-09-18 (R-B5-D2): added `batchOrganize.notice.revertLeftoverFolders` — the notice shown when a batch-organize revert leaves a created folder behind (kept because it is no longer empty, or the removal failed), so the "vault back to pre-batch shape" promise stays visible instead of silent.
...
> **Updated**: 2026-07-24 configuration completeness — added `settings.codex.approvalPolicy.*`, `settings.codex.tab.permissions`, `settings.codex.groups.permissions(Desc)`, `chat.sessionSettings.modal.codexApprovalPolicy(Desc)`, and resource-error keys `outside-allowlist` / `archive-failed` for both Claude and Codex.
...
2026-07-18：新增 `chat.composerPopover.*`，为共享 Composer popover frame 提供键盘提示、Agent / permission / model 标题与 model 搜索、加载和空态文案；这些键只提供卡片框架 copy，不改变既有 Agent 默认项、permission 描述或 backend-specific sandbox 字符串。
...
2026-07-16：新增 `settings.capabilityLab.tabs.*`，为 Claude Code、OpenCode、Codex manual-activation backend tabs 提供 tablist label、tab labels、state accessible name 和脱敏 panel load failure；原有 `settings.capabilityLab.backends.*` / `matrix.*` 继续提供 panel workspace 和矩阵文案。
...
Composer popover 新增 `chat.composerPopover.closeHint`，并与既有 navigate/select hint 共同构成 `↑↓ Navigate`、`Enter Select`、`Esc Close` 的共享卡片 footer。
...
2026-05-28 文档更正：移除关于 `chat.input.placeholderJsonSuffix` 与 backend-aware composer placeholder 已落地的表述。当前实现未引入该 i18n 键，也未在输入框占位符中追加 `/json` discoverability 文案；discoverability 通过 composer capability hint 落地（`chat.input.capabilityHint.jsonLabel` + `chat.input.capabilityHint.jsonTooltip`，Claude Code 和 Codex backend 对用户显示 `Structured reply`，点击后底层仍插入 `/json `，OpenCode backend 不显示），并且仍然只覆盖固定 schema trigger，不暗示任意 schema authoring。
...
## 2026-07-26 Claude configuration workbench locale keys
