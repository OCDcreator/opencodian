...
> 2026-09-18 (R-C6): 新增 `settings.security.tab.remote` 与 `settings.remoteControl.*` 键组（主开关/状态/绑定地址/非环回二次确认模态/令牌一次性展示与重生成确认/审计目录），文案诚实陈述风险：明文 HTTP、同网段嗅探、令牌等同驱动能力、审计无身份模型。
...
2026-09-18：新增 FlowText 对齐批次 A1 文案——`settings.inlineEdit.triggerAt.*`（@ 键唤起开关）、`settings.inlineEdit.hotkey.*`（命令快捷键发现性入口）、`settings.inlineEdit.presets.*` 与 `presetRow.*`（# 预设提示词管理）、内置预设目录 `inlineEdit.presets.{expand,condense,translate,summarize-table,polish-tone,fix-typos}.{label,prompt}`，以及菜单空态 `inlineEdit.presetMenu.empty`。
...
> **最近更新**: 2026-09-11 — 新增 `settings.model.iconCache.builtinPicker.library.modelsdev`，用于内置 provider 图标选择器新增的 models.dev 图库过滤项。
...
> **Updated**: 2026-07-29 — 新增插件版本管理折叠卡片的中英文展开/收起辅助标签文案。
...
> 2026-09-19 (按后端差异的用户可见声明)：与 en 同步新增两处纯声明键——`settings.inlineCompletion.latencyNotice.name` / `.desc`（补全首字节按后端差异：pi 预热 8/8 次 328–788ms；OpenCode 路由通常 0.9–1.3s 且部分模型不返回建议；并给出可操作建议）与 `settings.inlineEdit.streamPreviewNotice.name` / `.desc`（流式预览逐步增长同样取决于后端接缝：pi 按 delta 流式 → 4 个中间状态；OpenCode 路由下服务器不暴露部分文本 → 结束时一次出现，功能不受影响）。数字均为实测值。
