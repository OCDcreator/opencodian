...
> 2026-09-21 (advantage-parity R-F4)：`chatWarmSessionEnabled`（默认关、strict boolean load）复用唯一 `InlineCompletionService`；pool host 由 R-C3 / R-F4 任一开关启用。R-F4 启动、开关开启与两个 active-backend settings 入口均调 `prewarmExclusive()`，只构造一个空的只读 aux session；任一开关关闭仅在另一个也关闭时 `disposeAll()`，不破坏仍启用的一侧。
...
> 2026-09-18 (R-C6): `configureVaultScopedServices()` 末尾组合 `initRemoteControl(vaultPath)`（core.remotecontrol owner）：仅构造 `RemoteControlService`——注入窄会话驱动（绑定 `openCodeService.createSession/sendMessage/cancelStream` 既有公开 API，零改动 OpenCodeService）、动态 `knownSecrets`（remoteControlToken + 现存 apiKey/password/token，供审计 hardened redaction）、`vaultPath` 与 `notify`，随后 `applySettings()`（关闭态为不构造 server 的 no-op）；`onunload()` 增加 `remoteControlService?.dispose()`（关监听、中止在飞指令、flush 审计）。监听/鉴权/白名单逻辑零行驻留本文件。
> 2026-09-18 (R-C3-D1): `configureInlineCompletion()` 现在为 `InlineCompletionService`（池）与 `InlineCompletionController` 都传入必填的 `notify: (message) => new Notice(message)`——修复组合根漏配导致 R-C3 全部四条失败如实上报（sessionUnavailable / unsupportedAfterFailures / writeToolObserved / unsupported）静默丢失的缺陷；两处 options 的 `notify` 已改为必填，漏配在编译期失败，配套组合级回归测试 `tests/unit/main/inlineCompletionNotifyWiring.test.ts` 走真实装配断言消息可达。
...
> 2026-09-18 (FlowText parity R-A5/R-A6/R-A7): the inline-edit host bridge passes `maxConcurrentEdits` (`inlineEditMaxConcurrentEdits`) and `documentModeEnabled` (`inlineEditDocumentModeEnabled`) and implements `resolveContextFile` via `app.vault.getAbstractFileByPath()` with `instanceof TFile | TFolder` validation (vault-external paths resolve to nothing, so drops never become chips); `listInlineEditContextFiles` now enumerates folders (`getAllLoadedFiles`, skipping hidden/angle-bracket paths) alongside `md`/`txt` files; the controller is constructed with `confirmDocumentReplace: confirmInlineEditDocumentReplace(...)` (R-A6 second confirmation); a new `inline-edit-document` command (editor-check-gated on `canRunInlineEdit() && documentModeEnabled()`) and an editor-menu entry open the whole-document form; the `@` trigger now consumes only when `open()` actually returned true (multi-edit cap), and the selection affordance's `isEditing` uses `hasActiveEdits()`; `active-leaf-change`/`layout-change` prune detached inline edits (closed tabs / switched files dispose every edit and aux session).
...
- `src/shared/*`
- `src/utils/glass`
- `src/core/agents/backend/AgentAdapterWiring` (adapter 注册 wiring helper)
