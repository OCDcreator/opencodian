...
`createLogger(scope)` 会先通过 `resolveDebugModuleKey(scope)` 归到一个 debug module；如果调用方显式传 `moduleKey`，则优先用显式值。

### 最近日志缓存

- `recentLogEntries` 最多保留 `500` 条
- 只有真正输出的日志才会进入缓存
- `getRecentLogText()` 会输出 `[LEVEL] [moduleKey] [scope]` 头信息
...
| `getRecentLogEntries()` / `getRecentLogText()` | 读取最近日志缓存 |
