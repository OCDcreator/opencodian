...
后端无关的泛化 trace 持久化 store：`TraceStore<TEvent extends TraceEventBase = TraceEventBase>`。由 `OpenCodeTraceStore` 原样下沉泛化而来，私有逻辑（批量队列、retention、degrade、rebuildIndex、salt、exportTraceBundle）只依赖 `TraceEventBase` 已有字段。构造 options 支持 `bundlePrefix`、后端对应的 `runStartEventName`（Codex 注入 `turn.started`）、`disabled`（关闭时不初始化文件系统，之后由 `enable()` 惰性开启）与 `sanitizeExport`（导出 JSONL 写盘前的后端自定义 scrub）。`exportTraceBundle` 的目录前缀改为 `` `${bundlePrefix}-` ``；`fallbackDirectory` 缺省为 generic 兜底 `~/.config/obsidian/OpenCodian/diagnostics`（正常调用方总会显式传入）。
...
export class TraceStore<TEvent extends TraceEventBase = TraceEventBase> {
  constructor(customDirectory?: string, fallbackDirectory?: string, options?: { bundlePrefix?: string; runStartEventName?: string; disabled?: boolean; sanitizeExport?: (content: string) => string });
