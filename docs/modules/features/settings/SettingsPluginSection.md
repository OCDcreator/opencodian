...
`SettingsPluginSection` 是 settings/plugins 分区的厚 owner。它从 `OpenCodianSettings.ts` 接管插件管理 section 的完整 lifecycle：插件环境快照刷新、project config plugin 编辑器、项目插件安装区、per-entry 启用/禁用与删除控制、isolation mode 写回、项目插件目录创建，以及项目级 OMO 配置创建/打开。
...
- 创建 section heading 与 overview/global/project-directory/OMO 四个 subsection，其中 project directory 区包含项目插件 install section 与 managed entries
...
| `renderManagedEntryGroup()` | 渲染项目级 config / directory 插件的托管条目分组，包含空态、数量与禁用状态 |
...
- `PluginManagementService.ts`: 提供 plugin 快照、project plugin config 写回、project plugin install/uninstall/toggle/delete、project plugin directory 创建与 OMO config 创建
