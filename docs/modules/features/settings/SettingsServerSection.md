...
- connection/auth 里的普通 `Setting` 行不在本模块定义专属卡片色。它们继承 `settings-layout-contract.css` 的 Settings Form Row Card 规范：中性背景、细边框、左侧 label/description、右侧 Obsidian 原生 control column；错误只通过 Notice、文字或真实状态 panel 表达，不把整张普通 row 染成 warning/error tint。
