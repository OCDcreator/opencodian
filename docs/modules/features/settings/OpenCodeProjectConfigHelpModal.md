...
- `formatterLsp`：解释格式化器负责编辑后的代码风格整理，语言服务负责代码结构、引用和诊断上下文；说明默认、全部禁用和自定义模式的适用场景
...
- `SettingsFormatterSection.ts`：在 formatter / LSP mode setting 上挂 help button，并以 `formatterLsp` topic 打开本 modal
