...
- 通过 `SettingsCommandsSection` 协调 Commands section 的 runtime+project slash-command catalog 与 `hiddenSlashCommands` 用户可见性 lifecycle
...
- **Commands**
  ...
  - 当前只暴露 catalog + visible/hidden 开关，写回路径统一走插件设置 `hiddenSlashCommands`，为后续 command template 编辑、placeholder preview 与 slash execution slice 预留 catalog seam
- **Plugins**
...
- Commands section 的 runtime+project slash-command catalog 与 `hiddenSlashCommands` 可见性写回现在由 `SettingsCommandsSection` 持有
...
- `SettingsCommandsSection`: 管理 Commands section 的 runtime+project slash-command catalog 与 `hiddenSlashCommands` 用户可见性写回；更厚的 command 编辑 / placeholder / slash execution runtime 仍留给后续 commands slice
...
- 如果只是调整 Commands section 的目录加载或 `hiddenSlashCommands` 可见性写回，优先改 `SettingsCommandsSection`，不要再把这条 lifecycle 塞回主设置类。
