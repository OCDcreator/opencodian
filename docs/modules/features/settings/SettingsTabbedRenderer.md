...
> 2026-09-21 (advantage-parity R-F7)：deps 新增 `renderEnvironmentVariablesSetting`；General 合并块在 keychain 行之后渲染环境变量行。
