...
- Debug section 的 OpenCode 与 Codex diagnostics port factories；renderer 只在 composition 边界把 `createOpenCodeTraceDiagnosticsPort()` 与 `createCodexTraceDiagnosticsPort()` 注入 `SettingsDebugSection`
