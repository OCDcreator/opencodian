...
`describeCapabilityAvailability()` 是 Server/Conversation experimental gate 与 disclosure row 共用的格式化路径。它按 `reasonCode` 本地化 resolver 的固定 reason，保留没有 code 的动态脱敏文本，并组合 optional minimum-server hint；available capability 留给调用方显示本地的确认边界说明。
