...
- advanced custom CSS declarations textarea 还会通过 `TextareaSizeMemory` 以 `style-custom-css-declarations` 为稳定 key 记住用户手动调整后的高度；`dispose()` 时统一释放该 observer
...
| `dispose()` | 释放 shared-control binding、preset/background/input 子 owner，以及 advanced custom CSS textarea 的 size-memory observer |
...
- `TextareaSizeMemory.ts`: 为 advanced custom CSS textarea 提供高度恢复与 resize 持久化
...
- `dispose()` 不仅负责 shared-control binding 和子 owner，还负责清理 advanced custom CSS textarea 的尺寸记忆 observer，避免 settings 重建后重复持有旧 textarea
