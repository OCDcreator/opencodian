...
Provider 内置图标选择 Modal。面向单个 provider 展示 `LobeHub` 与 `OpenCode` 两套内置图标，支持搜索、库过滤、LobeHub variant 过滤、推荐排序、当前选中高亮，以及全局 provider 图标颜色模式（跟随系统 / 单色 / 彩色）的实时预览与保存；点击卡片后会把 `{ libraryId, iconId, variant }` 一起回调给 `ProviderIconCacheModal` 持久化。

## 核心逻辑

- 顶部控件包含库过滤下拉（全部 / LobeHub / OpenCode）、LobeHub variant 下拉（Auto / Mono / Color / Brand / …）与搜索框
