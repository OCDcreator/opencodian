...
- 新增 `renderToolsProofStatusNotice()`、`renderLimitsProofStatusNotice()` 和 `renderEnvProofStatusNotice()` 三个 compact proof-status notice 方法，在对应标签中渲染 runtime proof status；三个 notice 均使用 `data-proof-state="readback"`（runtime readback verified；env live behavior proof defers to Capability Lab diagnostic bypass）；样式位于 `src/style/components/settings-claude-code.css`，使用 `data-proof-state` 和 `data-claude-code-proof-status` 选择器
