...
`SettingsModelCatalogPresenter` 是 settings/model 目录展示的厚 owner。它把原本散落在 `OpenCodianSettings.ts` 里的 provider accordion、search、catalog summary card、bulk toggle、provider probe badge/detail 和过滤状态收束到一个专门 presenter，让 settings tab 主类回到“装配 host + 持久化写回”的角色。
