...
`onOpen()` 调用 `renderSettings()` 构建完整设置界面；`onClose()` 负责销毁 section owner、dropdown enhancer、滚动 coordinator 与待执行的 `requestAnimationFrame` 刷新。该 view 的滚动 coordinator 固定返回 `0`，不会复用标准 settings tab 的 `settingsPanelScrollTop`。
