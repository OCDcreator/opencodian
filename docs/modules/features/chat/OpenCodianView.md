...
- `TabConversationActivationBridge`：当前活动 tab 的 empty-state activation 与 current-tab 新建会话打开路径的消息区 shell cleanup、baseline commit，以及复用 `QuestionTodoActivationRefreshCoordinator` 的 question/todo/context/background-task 后续刷新编排
- `TabViewActivationBridge`：tab/pane activation 预刷新写回，以及 streaming / empty-tab activation outcome 的 selector、context identity、send-button 与 background-task indicator 刷新编排；question/todo dock writeback 已委托 `QuestionTodoActivationRefreshCoordinator`
