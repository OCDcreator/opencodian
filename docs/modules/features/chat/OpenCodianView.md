...
send pipeline 调试摘要函数（`summarizeContentBlocksForDebug`、`summarizeChatMessageForDebug`、`summarizeCoreStreamChunkForDebug`、`summarizeRenderedStreamChunkForDebug`）现在由 `runtime/SendPipelineDebugSummaries.ts` 承接。assistant finalization debug 的 allowlist gate check（`shouldLogAssistantFinalizationDebug`）和统一 log emitter（`logAssistantFinalizationDebug`）现在由 `services/trailingAssistantPatchDebug.ts` 承接。`OpenCodianView` 仅通过 import 调用这些纯函数，并在多个 host seam（`SendPipelineDebugPort`、`ConversationRenderHost`、`MessageFinalizationServiceHost` 等）中直接传递函数引用；view 本身不再保留这些私有实现。
