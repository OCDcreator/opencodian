...
构造函数接收 `plugin: ChatPluginPort`。`ChatPluginPort` 是 consumer-owned 的纯 type seam：不创建 runtime、不开启状态机，也不拥有 view/tab lifecycle。它通过 `Pick` 收窄 view 实际需要的 app/manifest、settings、OpenCode/config/model 服务、Claude/Codex UI context、settings tab 与 conversation commands；唯一保留的 concrete service 是带 private state 的 nominal `AgentServiceRegistry`，因为 backend routing helpers 需要按该类实例工作。其余依赖均为窄接口，不把完整插件对象作为上游事实暴露给本模块。

该 port extends 已存在的 `TabRuntimePluginSource`，唯一原因是 view 会把同一个 `plugin: this.plugin` 传入 tab runtime。由此复用既有的 UI-state save 与 cache-trim persistence contract（`saveSettingsUiStateImmediately()`、`scheduleSettingsUiStateSave()`、`trimConversationFullMessageCache()`）；这不是新增 forwarding layer 或 service locator。`OpenCodianView` 的构造、`onOpen()`/`onClose()` 以及 tab/session 生命周期语义保持不变。
...
- `services/ChatDiagnosticsCoordinator.ts` 的统一 diagnostics host seam：View 只提供 OpenCode/Codex/Claude settings、current conversation、active tab/session、header refresh、Menu、prompt、clipboard 与 Notice callbacks；View 持有一个由工厂创建的 coordinator。backend trace service getter 留在 app-composition 工厂闭包中，coordinator 构造并持有 Codex/Claude adapters；View 没有三个 backend trace service/store/report-builder 的直接访问，也不直接持有 Codex/Claude adapter
...
- `ConversationTabRuntimeCoordinator` 的 tab bar mutable state 与 persistence host wiring（通过 `createConversationTabRuntimeCoordinator` 顶层工厂一次性构造协调器；view 将 consumer-owned `ChatPluginPort` 作为同一 `plugin` 引用传入，工厂内部只分解既有 persistence contract）
