...
它不持有 `OpenCodianView`、`plugin` 或 tab 状态；调用方只传当前 catalog、filter、selection、用于解析本地图标资源路径的 `app`，以及回调。
...
- provider identity 仅在 heading 中渲染一次：一个 provider icon（`ProviderIconService.createIconElement(app, providerId)` 同步获取，走完整分阶段解析——别名表 / 本地 bundled / LobeHub / models.dev / 碰撞匹配，返回 null 时静默跳过）加一个紧凑文本标签
...
- configured-only option 仍留在 effective 列表中，显示 badge 与 tooltip 并设置 `aria-disabled="true"`，但不会获得 hover highlight、click callback 或预置 keyboard highlight
