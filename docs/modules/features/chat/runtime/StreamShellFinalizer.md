...
- 有 stream content blocks：追加时间戳与 copy button；如果 `LocalStreamOutcome.structuredOutput` 存在，还通过 `host.renderStructuredOutputIfPresent()` 在 message shell 上注入结构化输出 badge；随后调用 `suppressDuplicateStructuredOutputTextInDom()` 检查并移除最后一块重复的 `.streaming-text-block`
