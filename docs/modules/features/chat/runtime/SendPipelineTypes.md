# SendPipelineTypes
> 2026-09-21 (advantage-parity R-F1)：SendPipelinePreparationPort 增 notifyFollowUpQueueChanged（消费后队列条即时出队）。
...
> 2026-07-30: The same backend-neutral contract now carries Claude Code session-trace claims. `SendPipelineViewPort` exposes optional `claimClaudeDiagnosticRunToken(tabId, sessionId)` and `refreshClaudeDiagnosticsState(tabId)` hooks; `DiagnosticRunToken` remains structurally shared and no Claude-specific trace state is owned by the send pipeline.
