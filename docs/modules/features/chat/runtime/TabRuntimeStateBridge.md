...
它不推导 session status、background-task live predicate 或 question/todo attention 规则；这些判断仍分别留在 `BackgroundTaskLiveSignalCoordinator`、`BackgroundTaskPostSyncCoordinator`、`QuestionDockCoordinator` 与发送/收尾链路。本模块只负责把已经算好的 runtime state 写回 `TabManager` 与消息区 DOM。
