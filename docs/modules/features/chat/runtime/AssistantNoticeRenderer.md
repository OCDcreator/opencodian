...
`AssistantNoticeRenderer` 是发送 runtime 的 assistant notice 专用 helper。它把 stream error / interrupted notice 的 message 构造，以及 persisted / placeholder 两种 notice 的 card + footer 编排都从 `OpenCodianView` 中抽出；notice card 内部 DOM 组装交给 `AssistantNoticeCardRenderer`，notice footer 收尾则交给 `AssistantNoticeFooterFinalizer`。
...
- `renderPersistedAssistantNotice()`：渲染已创建好的 persisted notice shell，统一追加 notice class、card 与 footer 收尾
...
- 让 `OpenCodianView.renderMessage()` 的 persisted notice 分支退回到 host assembly，而不是自己编排 card + footer
...
- 这个模块仍通过 `AssistantNoticeRenderHost` 复用宿主提供的 notice card 渲染与 footer finalization；当前这层 host 由 `AssistantShellViewHostAdapter.ts` 统一装配，persisted / placeholder notice 都走同一条 seam。
