...
- `handleToolCallEnd()` 继续承接 todo tool 的即时 refresh；`isTodoTool()` 现在同时识别 OpenCode `todowrite` / `todoread` 与 Claude Code `TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet`（normalized case-insensitive canonicalization）作为 refresh trigger；native `task` 结束会写回 launch result，completed / error 状态会经由 `toolMetadata.sessionId` 优先生成 completion snapshot，然后重新 arm gate 并触发 indicator rerender
...
- `SessionTodoCoordinator` 继续负责 todowrite snapshot 写回、Claude Code Task* incremental CRUD model、默认 session 解析与 todo refresh，本 coordinator 只是透过 bundle 提供的 host 复用这些 port
