...
- `handleToolCallStart()` 先复用 todo snapshot host（通常落到 `SessionTodoRuntimeFacade.applyStreamingTodoSnapshotFromTool()`），再针对 `toolName === 'task'` 的流式调用补齐 `backgroundTaskStartedAt`、authoritative-sync gate、stale fingerprint 清理、launch upsert 与 indicator rerender
...
- `SessionTodoRuntimeFacade` 负责把 todowrite snapshot 写回和默认 session 解析收束到 shared session todo runtime bridge
