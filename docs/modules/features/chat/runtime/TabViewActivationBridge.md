...
- `ConversationViewStateService.loadConversation()` 也不再直接持有 composer/model/context usage 这段 hydration 尾部 writeback；status/question/todo lazy refresh 由 `QuestionTodoStatusRefreshCoordinator` 统一触发
