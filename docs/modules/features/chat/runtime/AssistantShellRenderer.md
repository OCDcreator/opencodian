...
`AssistantShellRenderer` 是发送 runtime 的 assistant shell 专用 adapter。它把 streaming assistant shell 的创建、persisted assistant shell 的基础装配、reveal、timestamp row 维护与 copy/timestamp 收尾，从 `OpenCodianView` 中抽成独立模块。

## 公开接口

- `createAssistantMessageElement()`：创建 streaming assistant shell，并在 tab runtime 里登记当前 `streamingMessageEl` / `streamingContentEl`
- `createPersistedAssistantMessageElement()`：为 persisted assistant / notice message 创建基础 shell，统一处理 turn-body 挂载、assistant class 与 `data-message-id` / `data-source-message-id`
...
- 这个模块只负责 assistant shell 基础装配，不负责 notice message 构造或 notice card DOM；notice 内容仍由 `AssistantNoticeRenderer.ts` 负责。
