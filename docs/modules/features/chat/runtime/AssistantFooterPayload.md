...
`AssistantFooterPayload` 是 persisted assistant footer 的参数组装 helper。它把 `OpenCodianView` 里传给 `AssistantShellRenderer.addTimestampWithCopyButton()` 的 timestamp、copy-content、model 与 status payload 收束成一个纯函数。

## 公开接口

- `buildPersistedAssistantFooterPayload()`：根据 persisted assistant `ChatMessage` 与可选 status label 返回 footer payload
- `PersistedAssistantFooterPayloadOptions`：收束 helper 输入的 `message` 与 `statusLabel`
...
- `statusLabel` 由调用方计算，本 helper 不判断 interrupted/stale 等 stream state
