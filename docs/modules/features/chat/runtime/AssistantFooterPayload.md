...
`AssistantFooterPayload` 是 persisted assistant footer 的参数组装 helper。它把传给 `AssistantShellRenderer.addTimestampWithCopyButton()` 的 timestamp、copy-content、model 与 status payload 收束成纯 helper，并由 `PersistedAssistantFooterFinalizer` 负责真正触发 renderer 调用。
...
- 让 `OpenCodianView` 不再直接依赖 `AssistantShellRenderer.addTimestampWithCopyButton()` 的 persisted footer 参数细节
