...
`SlashCommandMenuCatalogCache` 是 chat composer suggestion catalog 的加载缓存。它把较慢的 runtime `sdk.command.list()`、`sdk.app.skills()`、project command 配置、`.opencode/commands/**/*.md` markdown command 文件和 command-owned hidden agent 配置合并成 chat 侧 `SlashCommandMenuItem[]`，同时把同一 `sdk.app.skills()` 调用侧带的 runtime `app.agents()` 与 project agent config 投影成 composer agent sidecar。
...
- cache 会额外注入 synthetic `/compact` command menu item（除非同名 runtime/project entry 已存在或被 hidden commands 隐藏），让手动会话压缩能出现在 `/` autocomplete；实际执行不走普通 `session.command()`，而由 `SlashCommandExecutionService` 的专用路径处理。
