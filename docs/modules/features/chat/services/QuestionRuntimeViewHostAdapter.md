...
当前这层 adapter 也会直接复用已有稳定 runtime port：question resolution-card gate 直接读取设置，tab attention 写回直接委托 `TabRuntimeStateBridge`；question resolve 后的 sync/status follow-up 已收束到 `QuestionRuntimeHostAdapter` 内部的 post-resolution adapter，因此 view 自己只需要继续暴露更窄的 tab/runtime 读取与 scroll pin 能力。
...
- `QuestionRuntimeHostAdapter` 负责 question resolve 之后的 status refresh / sync follow-up host，本模块不再承接这组后处理端口
