...
> **源码**: `src/features/chat/services/QuestionDockWritebackFacade.ts`
> **状态**: [REVIEW]

## 概述

`QuestionDockWritebackFacade` 把上方 question dock 的 **tab attention 与 dock render writeback** 从 `QuestionDockCoordinator` 中收束到一个小型 facade，专门负责：

- 在 active tab 的 pending questions 被 enqueue、remove、clear 或 refresh 后统一清掉 `needsAttention` 并触发 dock 重绘
- 在后台 tab enqueue 后立即设置 `needsAttention`
