...
- `FocusContextRuntimeService` 只把实际 preview refresh 结果交给 `RetainedSelectionRuntimeCoordinator` 同步 retained-selection 状态
- polling、active editor 取样时机、composer pointer/focus handoff 与 highlight cleanup 已下沉到 `RetainedSelectionRuntimeCoordinator`
- preview retain 规则继续由 `resolveFocusContextPreview()` 驱动，但 retained-selection runtime 不再与 MarkdownView fallback / preview debounce 逻辑混在同一个类里
...
- **RetainedSelectionRuntimeCoordinator**：持有 retained-selection polling、composer handoff 入口与 highlight service 生命周期
- **RetainedSelectionHighlightService**：经由 retained-selection runtime coordinator 持有 handoff/highlight state，并负责 CodeMirror/DOM 高亮 writeback
...
- service 只管理 editor/runtime 侧的 focus context；`MarkdownView` fallback 留在 `FocusContextMarkdownViewLocator`，附件构建留在 `ContextAttachmentBuilder`，chips 编排留在 `ComposerContextCoordinator`，retained-selection polling/highlight 留在 `RetainedSelectionRuntimeCoordinator`，active-tab preview state 写回则经由 `FocusContextViewHostAdapter`
