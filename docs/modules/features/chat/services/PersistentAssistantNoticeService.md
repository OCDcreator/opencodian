...
它不决定 notice 何时出现，也不生成 session todo / background task 的 fingerprint 与文案；这些规则仍分别由 `SessionTodoStateService`、`BackgroundTaskNoticeStateService` 与 `BackgroundTaskCompletionNoticeService` 持有。
