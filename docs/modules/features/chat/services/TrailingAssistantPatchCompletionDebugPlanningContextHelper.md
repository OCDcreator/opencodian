...
- 在 helper 内部改为读取 `tailStatePlan.shouldStickToBottom`，并把 tail-message summary 委托给 `TrailingAssistantPatchCompletionDebugSummaryPlanHelper`
...
- `TrailingAssistantPatchCompletionDebugPlanningContextSourceHelper` 负责把 tail-outcome `planningContext`、`tailStatePlan` 与摘要函数拼成这里消费的 source
