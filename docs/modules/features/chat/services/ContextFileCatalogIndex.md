...
`ContextFileCatalogIndex` 是 context catalog build/mutation 流程使用的可变索引。它集中处理 context-file 条目的资格过滤、排序、扩展名桶重算，以及 create/delete/rename 增量更新，让 `ContextFileCatalogBuildRunner` 只负责批量扫描，让 `ContextFileCatalogService` 只保留缓存与 build promise 协调。
...
- `finalizeBuild()` 在批量扫描结束后统一排序并重算扩展名桶，避免 `ContextFileCatalogBuildRunner` 或 `ContextFileCatalogService` 再持有条目规范化细节
...
- **ContextFileCatalogBuildRunner**：负责在 build 阶段批量调用 append/finalize
- **ContextFileCatalogService**：负责持有 runner/index，并把 vault 事件直接转成 index mutation
