...
R-C4 三期视图集成（feature.chat-services，设计 §3.3）：扫描 `pdf` 叶子并对每叶做运行时确认门（feature-detect 内部结构 + 实际调用一次原生序列化验证），按 `core.pdf` 的 `resolvePdfIntegrationLevel` 落 A/B/C 级；A/B 级向 `toolbar.toolbarRightEl/LeftEl` 挂 `clickable-icon` 按钮。选区捕获按证据升降级：只要序列化器存在就以 `child.getTextSelectionRangeStr({ win, contains })` 尝试原生序列化——`contains` 是 `document.contains` 的文档级包含谓词（bundle 核实宿主实现以 ctx 为包含根走 `e.contains(t)`，实测 `document.contains` 在真实选区上工作、`{win}` 单参在真实选区上必抛 `e.contains is not a function`、`pdfViewer.contains` 返回 null）；真实捕获成功即把 bridge 升 A（原因逐字 `native serializer proven on a live selection`），真实捕获失败即把已声称的 A 降 B（原因 `native range serializer failed on a live selection — downgraded to #page links`），失败时仍回退 B 级 DOM `getSelection()` + `data-page-number` 页锚定，用户永不空手。命令 `pdf-ask-selection` 捕获选区 → `pdf_selection` 条目 → 打开聊天；无可捕获选区时诚实降级 C 级（开聊天提示手动粘贴）。命令 `pdf-save-annotation` 取会话中最近一次带 PDF 上下文的问答 → 预览模态 → `applyAnnotationWrite` 写侧车（本 feature 唯一新写路径）：`beginBatchCapture` → 单次 `vault.process`/`vault.create` → `notePluginWrite` → `endBatchCapture`（R-B3 覆盖），A 级且 `highlightText` 可用时做临时高亮反馈。

首次探测不是终局（实机验收修复）：Obsidian 在叶子出现之后才逐步构建 viewer 内部结构（`viewer.child.pdfViewer`、toolbar 容器），首次探测几乎必然落在空 viewer 上。C 级叶子因此按有界退避（默认 250ms→4s，共 ~7.75s，可用 `PdfChatIntegrationOptions.viewerReadyRetryDelaysMs` 注入）+ 每次 workspace sync 重新探测，仅当新探测真实证明更高一级时才升级（探测永无真实选区，故探测只能给到 B——A 只能由真实捕获授予）；从不静默降级已证明的更高级；viewer 始终不可用的叶子保持 C 级及真实原因。`unmountBridge` / 清扫循环 fail-safe：无按钮（C 级/挂载失败）或畸形 bridge 拆除不抛错，单个坏 bridge 不会中断整轮清扫（曾因此阻断上述自愈）。
