...
它不触碰 `ConversationRenderHost`、DOM、分支专有的 log-plan wrapper 或具体的 payload 细节；只负责共享的 “logging context → planning context → payloadPlan → final log plan” 协调骨架。
...
- service 不再直接触碰 coordinator，而是把现成 logging context 交给 log-plan helper
