...
- `handleSignalSyncComplete()` 保留原有 signal-sync authoritative mark，随后调用 `BackgroundConversationPostSyncRefreshExecutor.refreshSignalSyncedBackgroundConversation()` 并传递 `tabHasBackgroundTask`
...
- `OpenCodianView` 现在通过 `QuestionTodoBackgroundTaskRefreshHostAdapter` 只把 authoritative mark host bridge 接到本 coordinator；attention writeback 则改由 `BackgroundConversationAttentionCoordinator` 单独承接
...
- `BackgroundTaskPostSyncCoordinator` 负责 hidden signal/background-tab sync，以及 active visible-conversation background sync 之后的 visible/background refresh routing 与 signal authoritative mark
- 这让本轮继续沿着 master plan 的 P2 `question / todo / background task` lane，把 visible refresh、background execution、visible state-commit 与 background attention 判定保持在四个稳定边界，而不是继续散落在 view 的多个 sync 入口中
