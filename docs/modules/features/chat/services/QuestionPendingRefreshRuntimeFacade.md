...
它不负责向 OpenCode 拉取 pending question、按 session 过滤、pending refresh 完成后的 attention/render writeback、dock DOM render、dock waiter 生命周期、回答/拒绝请求，或 resolve 后的 status/sync follow-up；这些仍分别留给 `QuestionDockCoordinator`、`QuestionPendingRefreshWritebackFacade`、`QuestionDock`、`QuestionDockQueueRuntimeFacade`、question API 与 `QuestionPostResolutionRuntimeFacade`。
...
- `QuestionDockCoordinator` 继续负责 question API fetch、session 过滤与 dock resolution flow；dock queue 的 waiter/enqueue/remove runtime 读写由 `QuestionDockQueueRuntimeFacade` 承接，pending refresh 期间的 resolved-state / stale-state 读写由本 facade 承接，refresh 完成后的 attention/render writeback 由 `QuestionPendingRefreshWritebackFacade` 承接
