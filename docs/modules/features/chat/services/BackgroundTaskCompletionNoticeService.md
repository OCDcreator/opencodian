...
- 聚合同一 anchor 下的 completion reminder，并在 service 内部按 tab runtime 维护 queued notice state
...
- queued notice 不再挂在 `OpenCodianView.TabRuntimeState` 上，而是由 service 通过 tab runtime key 自己维护，避免 view 继续持有 queue 细节
...
- `BackgroundTaskCompletionNoticeService` 负责 completion notice queued state、content/fingerprint 和 persisted dedupe/append 协调；`OpenCodianView` 不再保存 `queuedBackgroundTaskCompletionNotices`
