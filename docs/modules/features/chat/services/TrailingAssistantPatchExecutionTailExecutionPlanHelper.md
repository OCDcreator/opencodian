...
- 接收由 service 预先算好的 `shouldFinalizeFooterOnly` 决策
...
- `ConversationRenderService` 现在只保留正文签名比较与 `shouldFinalizeFooterOnly` 决策，再把窄的 execution-tail planning-context 交给这里
