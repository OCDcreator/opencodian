...
- 接收由 service 读取 host body signature 后、再经 `TrailingAssistantPatchFooterFinalizationDecisionHelper` 预先算好的 `shouldFinalizeFooterOnly` 决策
...
- `ConversationRenderService` 现在只保留读取前后 body signature、调用 `TrailingAssistantPatchFooterFinalizationDecisionHelper` 得到 `shouldFinalizeFooterOnly`，再把窄的 execution-tail planning-context 交给这里
