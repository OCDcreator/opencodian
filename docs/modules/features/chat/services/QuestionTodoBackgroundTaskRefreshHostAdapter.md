...
- 从单一 `QuestionTodoBackgroundTaskRefreshViewHost` 派生 `QuestionTodoActivationRefreshBridge`、`BackgroundConversationPostSyncRefreshExecutor`、`VisibleConversationPostSyncStateCoordinator`、`VisibleConversationPostSyncCoordinator`、`BackgroundConversationSignalSyncStateCoordinator`、`BackgroundConversationAttentionCoordinator`、`BackgroundConversationPostSyncHandoffCoordinator` 七段链路需要的 host 回调，以及 dedicated background-task post-sync refresh port
- 让 activation-side supplemental refresh 与 background-task rebuild/completion follow-up、visible sync state-commit、background signal state、background attention 判定复用同一份 view bridge，同时把 visible question/todo post-sync refresh host 装配下沉到 `PostSyncQuestionTodoRefreshHostAdapter`
...
- `createQuestionTodoBackgroundTaskRefreshHosts()` 从同一份 view host 派生 activation refresh host、四组 background/visible handoff host 外加一条 dedicated post-sync refresh port，避免 `OpenCodianView` 继续维护多段闭包工厂和额外 background-task effect wiring
...
- `createQuestionTodoBackgroundTaskRefreshServices()` 先复用 `PostSyncQuestionTodoRefreshHostAdapter` 提供的 `QuestionTodoStatusRefreshCoordinator` / `PostSyncQuestionTodoRefreshPlanBuilder` / `PostSyncQuestionTodoRefreshFacade`，再顺序实例化 `QuestionTodoActivationRefreshBridge` → `BackgroundConversationPostSyncRefreshExecutor` → `VisibleConversationPostSyncStateCoordinator` → `VisibleConversationPostSyncCoordinator` → `BackgroundConversationAttentionCoordinator` → `BackgroundConversationSignalSyncStateCoordinator` → `BackgroundConversationPostSyncHandoffCoordinator`
