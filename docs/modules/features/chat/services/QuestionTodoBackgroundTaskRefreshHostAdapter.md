...
- 从单一 `QuestionTodoBackgroundTaskRefreshViewHost` 派生 `QuestionTodoStatusRefreshCoordinator`、`PostSyncQuestionTodoRefreshPlanBuilder`、`PostSyncQuestionTodoRefreshFacade`、`VisibleConversationPostSyncStateCoordinator`、`BackgroundTaskPostSyncCoordinator` 五段链路需要的 host 回调，以及 dedicated background-task post-sync refresh port
...
- `createQuestionTodoBackgroundTaskRefreshHosts()` 从同一份 view host 派生五组 host 外加一条 dedicated post-sync refresh port，避免 `OpenCodianView` 继续维护多段闭包工厂和额外 background-task effect wiring
...
- `createQuestionTodoBackgroundTaskRefreshServices()` 顺序实例化 `QuestionTodoStatusRefreshCoordinator` → `PostSyncQuestionTodoRefreshPlanBuilder` → `PostSyncQuestionTodoRefreshFacade` → `VisibleConversationPostSyncStateCoordinator` → `BackgroundTaskPostSyncCoordinator`
...
- 五个对象仍保持原有职责边界，但它们的 host wiring 不再散落在 view 构造函数
...
- `QuestionTodoStatusRefreshCoordinator`、`PostSyncQuestionTodoRefreshPlanBuilder`、`PostSyncQuestionTodoRefreshFacade`、`VisibleConversationPostSyncStateCoordinator` 与 `BackgroundTaskPostSyncCoordinator` 的业务边界保持分离
