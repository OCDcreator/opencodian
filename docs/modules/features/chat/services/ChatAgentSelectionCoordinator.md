...
- `mount()` 创建既有 trigger / dropdown，并把 Agent 内容挂到共享 `ComposerPopoverFrame` 的 content slot；frame 统一提供本地化标题、`Esc` 键帽和 Navigate / Select footer，Agent 仍独立拥有列表标题、OpenCode default 与候选行
...
- 鼠标打开不会抢走 composer 焦点；用 Enter、Space、ArrowDown 或 ArrowUp 打开时，异步 catalog settled 后会聚焦当前选中行（无选中候选时为 OpenCode default）。列表内 ArrowUp / ArrowDown 首尾循环，Enter 通过既有 `selectAgent()` 选择，Escape 关闭并把焦点还给 Agent trigger
