...
其中 conversation-sync runtime port 现在由 `TabActivationConversationSyncPortProvider` 预先收束 fingerprint read/write 与 loop-control seam，再由本模块与其他 activation ports 一起交给 factory。这样 `OpenCodianView` 不再直接维护 grouped factory-host 结构，只保留更薄的 late-bound runtime seam；既有 `TabActivationRuntimeViewHostFactory` 与 `TabActivationRuntimeHostAdapter` 继续负责 shared host assembly 与 bridge host 派生。
...
- `TabActivationConversationSyncPortProvider` 负责 conversation-sync fingerprint / loop-control regrouping
