...
`QuestionTodoActivationRefreshCoordinator` 把 activation/open 路径里反复出现的 **question dock render + session todo dock writeback + activation refresh handoff** 收束成一个更窄的 P2 coordinator。它专门负责：
...
- `QuestionTodoStatusRefreshCoordinator` 现在直接负责 activation/open 侧 supplemental status/question/todo refresh，而本 coordinator 保持为 dock render/writeback 顺序 owner
