...
- 在 streaming activation、loaded-conversation post-render、current-tab open 之后统一触发 todo dock render → question dock render → `QuestionTodoActivationRefreshBridge.refreshAfterActivation()`
...
- `QuestionTodoActivationRefreshBridge` 负责 activation/open 侧 supplemental status/question/todo refresh，避免本 coordinator 继续依赖 post-sync coordinator
