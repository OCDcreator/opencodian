...
它不负责 background task timeline 推导、inline panel DOM 渲染、completion notice 队列，也不直接操作 notice 内容；这些能力仍由 `OpenCodianView`、`BackgroundTaskNoticeStateService` 和 `BackgroundTaskCompletionNoticeService` 分别承接。
...
- `OpenCodianView` 仍负责 background task segment/timeline 推导、inline panel DOM 渲染、completion notice queue/flush 时机，以及具体 notice / reset host bridge
