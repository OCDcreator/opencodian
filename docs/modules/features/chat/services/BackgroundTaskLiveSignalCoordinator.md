...
export class BackgroundTaskLiveSignalCoordinator {
  constructor(
    sessionTodoStateService: BackgroundTaskLiveSignalTodoPort,
    timelineService: BackgroundTaskLiveSignalTimelinePort,
    noticeStateService: BackgroundTaskLiveSignalNoticePort,
...
- `OpenCodianView` 只保留扁平的 tab runtime / session lookup / view writeback seam；grouped host assembly 已并回 `BackgroundTaskLiveSignalCoordinator` 模块自身的 host builder，view 不再内联调用 `createBackgroundTaskLiveSignalCoordinatorHost()`
