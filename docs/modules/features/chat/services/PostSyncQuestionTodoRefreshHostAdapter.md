...
它不拥有 activation-side refresh、background-task writeback、visible sync state commit、authoritative mark 或 tab attention 判定；这些职责仍分别留在 `QuestionTodoActivationRefreshBridge`、`BackgroundConversationPostSyncRefreshExecutor`、`VisibleConversationPostSyncCoordinator`、`BackgroundConversationSignalSyncStateCoordinator` 与 `BackgroundConversationAttentionCoordinator`。
