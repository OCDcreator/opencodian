...
它不负责 background task timeline 推导、inline panel 渲染、completion reminder 队列，也不接管 `BackgroundTaskTimelineService` / `BackgroundTaskInlinePanelRenderer` / `OpenCodianView` 的 sync / hydration 时机判断。
...
- `OpenCodianView` 负责 hydration gate host bridge 与 completion notice 队列触发
