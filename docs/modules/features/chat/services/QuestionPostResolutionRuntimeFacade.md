...
- `QuestionRuntimeHostAdapter` 负责把共享的 tab/session runtime 读取、`SessionTodoCoordinator` 与 `ConversationSyncBridge` 组合成本 facade 所需的稳定 runtime host，并把它接入 question runtime bundle
