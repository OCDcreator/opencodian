...
- 把 source 到 tail-state inputs 的装配委托给 `TrailingAssistantPatchTailStatePlanningContextInputsHelper`
- 把 inputs 到最终 planning-context shape 的装配委托给 `TrailingAssistantPatchTailStatePlanningContextShapeHelper`
...
- `TrailingAssistantPatchTailStatePlanningContextInputsHelper` 负责把 tail-outcome source fields 收束成 tail-state inputs
- `TrailingAssistantPatchTailStatePlanningContextShapeHelper` 负责把 inputs 纯装配成最终 tail-state planning-context shape
