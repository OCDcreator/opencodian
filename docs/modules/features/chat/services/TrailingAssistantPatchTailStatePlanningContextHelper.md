...
- 保持 `buildTrailingAssistantPatchTailStatePlan()` 下游继续消费同一份稳定的 tail-state contract
...
- `ConversationRenderService` 现在直接把 tail-outcome `planningContext` 交给这里，不再在 service 内自行拼装 tail-state inputs
...
- `ConversationRenderService` 里的 `buildTrailingAssistantPatchTailStatePlan()` 继续消费这里返回的窄 planning-context，并把最终 `tailStatePlan` 交给 `TrailingAssistantPatchTailStateApplierHelper`
