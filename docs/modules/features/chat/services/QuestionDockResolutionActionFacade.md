...
- `submit` 成功时返回带 `answers` 与 answered `QuestionResolution` 的 `reply` action
- `reject` 不读取 draft answers，直接返回 rejected `QuestionResolution`，避免拒绝路径被答案 runtime 状态影响
...
- `QuestionDockCoordinator` 只消费本 facade 的 `skip` / `answer-required` / `reply` / `reject` action，再负责保留真实 API 调用、错误 notice 与 queue removal writeback 顺序
