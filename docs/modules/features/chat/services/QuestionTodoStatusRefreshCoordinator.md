...
它不直接调用 OpenCode API，也不拥有 question dock DOM、todo/status snapshot state 或 background-task timeline rebuild；这些仍分别由 `QuestionDockCoordinator`、`SessionTodoStatusRefreshService`、`SessionTodoStateService` 与 `BackgroundTaskTimelineService` 承接。本模块只负责 activation/post-sync 场景里的组合刷新顺序与 runtime gate。
...
- `refreshAfterPostSync()` 保持 post-sync 语义：先等待 pending-question refresh，再运行 `afterPendingQuestionRefresh` hook，让 background-task runtime rebuild 仍发生在 todo/status gate 之前
...
- `BackgroundTaskPostSyncCoordinator` 现在只负责 authoritative mark、timeline rebuild hook、completion notice、attention 与 state-commit 判定，pending-question + todo/status refresh order 由本 coordinator 承接
- current-tab conversation open fast path 同样复用本 coordinator，避免 `OpenCodianView` 再内联三段刷新调用
- 这条边界推进的是 master plan 的 P1 `tab / pane / conversation activation 与 sync orchestration` lane：把 activation/post-sync 的组合刷新 ownership 从 view/bridge host surface 中继续下沉
