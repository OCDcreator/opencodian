...
- 接收已经由上游 helper 通过 `TrailingAssistantPatchFooterFinalizationDecisionSourceContractHelper` 读取好的前后正文签名字符串
...
- `TrailingAssistantPatchFooterFinalizationExecutionTailDecisionHelper` 现在负责把 source-contract helper 与这里串联起来，对外暴露最终布尔决策
- `TrailingAssistantPatchExecutionTailPlanningContextHelper` 继续提供 `previousTailMessage` / `nextTailMessage` 给 source-contract helper 读取签名
- `ConversationRenderService` 现在只负责把 host getter 注入到 execution-tail decision helper，并消费最终布尔决策
