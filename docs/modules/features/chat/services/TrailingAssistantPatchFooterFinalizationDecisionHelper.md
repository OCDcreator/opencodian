...
- 接收已经由 service 从 host 读取好的前后正文签名字符串
- 只根据签名是否相等返回 `shouldFinalizeFooterOnly`
- 让 `ConversationRenderService` 退出正文签名相等性判断本身，只保留 host 读取与传参职责
...
- `ConversationRenderService` 现在先通过 `host.assistantTailRender.getBodySignature()` 读取前后正文签名，再把字符串交给这里比较
