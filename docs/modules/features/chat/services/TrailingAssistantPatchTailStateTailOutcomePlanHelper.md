...
`TrailingAssistantPatchTailStateTailOutcomePlanHelper` 把 trailing-assistant tail-outcome 路径里“tail-outcome planning-context → tailStatePlan”的纯编排从更上游的 success-plan 组合链中抽成了独立 helper：
...
- `TrailingAssistantPatchTailOutcomeExecutionTailPlanHelper` 现在只把共享的 tail-outcome planning-context 交给这里，不再让更上游模块直接串联 tail-state planning-context 与最终 `tailStatePlan` shape
