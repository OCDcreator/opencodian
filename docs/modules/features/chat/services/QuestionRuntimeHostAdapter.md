...
- 从同一份 host 继续派生 `QuestionDockQueueRuntimeFacade` 与 `QuestionPendingRefreshRuntimeFacade` 所需的 dock-queue / pending-refresh runtime state port，并把 pending-refresh attention/render writeback 接到 `QuestionPendingRefreshWritebackFacade`
- 顺序装配 inline question card、resolved-question runtime、dock-queue runtime facade、pending-refresh runtime facade、pending-refresh writeback facade、post-resolution runtime facade、resolution writeback facade、上方 question dock，以及 `QuestionResolutionFlowCoordinator` 九个协作模块，避免 view 继续维护多段 `create*Host()` 和散落的 resolve flow instantiation
...
- `createQuestionRuntimeHosts()` 从同一份 view host 派生 inline-card、resolution、dock、dock-queue runtime、pending-refresh runtime 与 post-resolution runtime 六组 host，避免 `OpenCodianView` 继续为 question 子链维护多段闭包工厂
- `createQuestionRuntimeServices()` 顺序实例化 `QuestionInlineCardRenderer` → `QuestionResolutionCoordinator` → `QuestionDockQueueRuntimeFacade` → `QuestionPendingRefreshRuntimeFacade` → `QuestionPostResolutionRuntimeFacade` → `QuestionPendingRefreshWritebackFacade` → `QuestionResolutionWritebackFacade` → `QuestionDockCoordinator` → `QuestionResolutionFlowCoordinator`，保留原来的协作依赖关系，并把 dock / inline resolve flow 的 resolved-request suppression 接到同一条 writeback seam
...
- pending-question refresh 的 resolved-request suppression 与 runtime map pruning 不再留在 `QuestionDockCoordinator` 的 refresh 分支里铺开；refresh 完成后的 attention/render writeback 也由 adapter-wired `QuestionPendingRefreshWritebackFacade` 承接；dock 与 inline resolve 都通过 adapter-wired `QuestionResolutionWritebackFacade` 标记 suppression
