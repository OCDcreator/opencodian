...
- 从单一 `QuestionRuntimeViewHost` 派生 `QuestionInlineCardRenderer`、`QuestionInlineResolutionActionFacade`、`QuestionResolutionCoordinator`、`QuestionDockRenderStateFacade`、`QuestionDockResolutionActionFacade`、`QuestionResolutionExecutionFacade` 与 `QuestionDockCoordinator` 所需的 host 回调
...
- `createQuestionRuntimeServices()` 现在把 pending-question refresh、queue waiter、draft/active selection、attention/render writeback、resolved-id suppression 与 execute-then-writeback 顺序都交给 `QuestionDockCoordinator`
...
- send pipeline 触发 question request 时，`QuestionResolutionFlowCoordinator` 只选择 dock 或 inline action source，最终 apply/writeback 都回到同一个 `QuestionDockCoordinator`
