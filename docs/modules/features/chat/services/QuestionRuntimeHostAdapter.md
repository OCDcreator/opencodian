...
- 把 question resolve 之后的 status refresh / conversation sync follow-up 改为接收单独的 `QuestionPostResolutionRuntimeFacadeHost`，不再让通用 `QuestionRuntimeViewHost` 背负这组后处理端口
...
当前这份 `QuestionRuntimeViewHost` 通常先由 `QuestionRuntimeViewHostFactory` + `QuestionRuntimeViewHostAdapter` 两层准备：factory 先从 view 收口 late-bound 的 question 相邻端口，adapter 再把 dock/settings/API/tab-attention 组合成稳定 host；question resolve 后的 status refresh / sync follow-up 现在改由独立的 `QuestionPostResolutionRuntimeHostAdapter` 复用已有 `SessionTodoStatusRefreshService` 与 `ConversationSyncBridge` 稳定 port。
...
- `createQuestionRuntimeHosts()` 从同一份 view host 派生 inline-card、inline-resolution-action、resolution、dock、dock-render-state、dock-resolution-action、resolution-execution、dock-refresh、dock-queue runtime 与 pending-refresh runtime 十组 host，并把 post-resolution runtime host 改为外部注入，避免 `OpenCodianView` 继续为 question 子链维护一份过宽的共享 view host
...
- dock 与 inline resolve 成功后的 runtime follow-up 也不再分别内嵌在 coordinator 中，而是由 `QuestionResolutionApplyFacade` 统一串到 `QuestionResolutionWritebackFacade`，再由后者复用独立注入的 `QuestionPostResolutionRuntimeFacade.followUpAfterResolution()`
...
- `OpenCodianView` 现在只提供一份更窄的 `QuestionRuntimeViewHostFactoryHost`；late-bound 的 dock/API/tab-attention wiring 先交给 `QuestionRuntimeViewHostFactory`，再由 `QuestionRuntimeViewHostAdapter` 组合成 `QuestionRuntimeViewHost`，post-resolution follow-up 则单独交给 `QuestionPostResolutionRuntimeHostAdapter`
