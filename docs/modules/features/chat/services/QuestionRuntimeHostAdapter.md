...
- 从同一份 host 继续派生 `QuestionDockQueueRuntimeFacade` 与 `QuestionPendingRefreshRuntimeFacade` 所需的 dock-queue / pending-refresh runtime state port，并把 resolved-request suppression 接入共享的 `QuestionResolutionWritebackFacade`
- 顺序装配 inline question card、resolved-question runtime、dock-queue runtime facade、pending-refresh runtime facade、post-resolution runtime facade、resolution writeback facade、上方 question dock，以及 `QuestionResolutionFlowCoordinator` 八个协作模块，避免 view 继续维护多段 `create*Host()` 和散落的 resolve flow instantiation
...
- `createQuestionRuntimeServices()` 顺序实例化 `QuestionInlineCardRenderer` → `QuestionResolutionCoordinator` → `QuestionDockQueueRuntimeFacade` → `QuestionPendingRefreshRuntimeFacade` → `QuestionPostResolutionRuntimeFacade` → `QuestionResolutionWritebackFacade` → `QuestionDockCoordinator` → `QuestionResolutionFlowCoordinator`，保留原来的协作依赖关系，并把 dock / inline resolve flow 的 resolved-request suppression 接到同一条 writeback seam
...
- pending-question refresh 的 resolved-request suppression 与 runtime map pruning 不再留在 `QuestionDockCoordinator` 的 refresh 分支里铺开；dock 与 inline resolve 都通过 adapter-wired `QuestionResolutionWritebackFacade` 标记 suppression
