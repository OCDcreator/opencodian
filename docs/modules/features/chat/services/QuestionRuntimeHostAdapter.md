...
- 从同一份 host 继续派生 `QuestionDockQueueRuntimeFacade` 与 `QuestionPendingRefreshRuntimeFacade` 所需的 dock-queue / pending-refresh runtime state port，并把 resolved-request suppression 作为小 port 直接交给 inline resolve flow
...
- pending-question refresh 的 resolved-request suppression 与 runtime map pruning 不再留在 `QuestionDockCoordinator` 的 refresh 分支里铺开；dock resolve 由 dock coordinator 委托，inline resolve flow 则通过 adapter-wired `QuestionPendingRefreshRuntimeFacade` 小 port 直接标记 suppression
