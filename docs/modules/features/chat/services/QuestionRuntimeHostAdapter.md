...
- 在同一 owner 内把 question resolve 之后的 status refresh / conversation sync follow-up 适配成 `QuestionPostResolutionRuntimeFacadeHost`，再和 resolved-id 标记 / resolved-card apply 一起并入共享 `QuestionResolutionExecutionFacade` lifecycle seam，避免通用 view host 背负这组后处理端口
...
- `createQuestionPostResolutionRuntimeHostAdapter()` 把 tab/session runtime 读取、status refresh 与 visible conversation sync port 组合成本模块内部使用的 post-resolution runtime host
- `createQuestionRuntimeHosts()` 从同一份 view host 派生 inline-card、inline-resolution-action、resolved-card runtime、dock lifecycle、dock-render-state、dock-resolution-action 与 resolution-execution 七组 host，并接入 post-resolution runtime host
