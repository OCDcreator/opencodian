...
- 从同一份 host 继续派生 `QuestionDockQueueRuntimeFacade` 与 `QuestionPendingRefreshRuntimeFacade` 所需的 dock-queue / pending-refresh runtime state port
- 顺序装配 inline question card、resolved-question runtime、dock-queue runtime facade、pending-refresh runtime facade、上方 question dock、post-resolution runtime facade，以及 `QuestionResolutionFlowCoordinator` 七个协作模块，避免 view 继续维护多段 `create*Host()` 和散落的 resolve flow instantiation
...
- `createQuestionRuntimeServices()` 顺序实例化 `QuestionInlineCardRenderer` → `QuestionResolutionCoordinator` → `QuestionDockQueueRuntimeFacade` → `QuestionPendingRefreshRuntimeFacade` → `QuestionPostResolutionRuntimeFacade` → `QuestionDockCoordinator` → `QuestionResolutionFlowCoordinator`，保留原来的协作依赖关系，并把 inline resolve flow 也收束回同一份 runtime bundle
...
- pending-question refresh 的 resolved-request suppression 与 runtime map pruning 不再留在 `QuestionDockCoordinator` 的 refresh 分支里铺开，而是通过 adapter-wired `QuestionPendingRefreshRuntimeFacade` 承接
...
- `QuestionDockCoordinator` 继续负责 pending-question API refresh/session filter、dock callbacks 与 resolve flow；dock queue runtime map 维护由 `QuestionDockQueueRuntimeFacade` 接管，pending refresh runtime map 维护由 `QuestionPendingRefreshRuntimeFacade` 接管，resolve 后的 runtime follow-up 由 `QuestionPostResolutionRuntimeFacade` 接管
