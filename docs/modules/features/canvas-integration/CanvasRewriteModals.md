...
- **`CanvasRewritePreviewModal`**：写前预览确认门——原文/新文对照 + 写入目标说明；Mermaid 单块结果附带实时渲染预览（`MarkdownRenderer.render` + 独立 `Component` 生命周期，渲染失败保留原始代码块）；`copyOnly`（D 级）把确认按钮换成"复制到剪贴板"并显式说明；`revertNote` 诚实展示撤销/回退边界（文本节点 `setData` 不在 R-B3 覆盖内且 Ctrl+Z 未验证；文件节点无回退面时拒绝语义由控制器执行）。
