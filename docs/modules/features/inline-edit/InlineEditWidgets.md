...
- `InlineEditPreviewWidget`：`Decoration.replace` 覆盖选区（插入形态为零长度 block widget），DOM 内渲染 diff span 与页脚。页脚左侧为身份标签（sparkles 图标 + 功能名 `inlineEdit.command.name`），右侧按钮顺序固定为**拒绝在前（`is-reject` 幽灵）、接受在后（`is-accept` 实心）**——主操作居右；测试与自动化按类名选择，不要按下标。预览 body 继承编辑器字体字号（展示的是正文内容）
