...
- 澄清通道防泄漏（§6.4/§6.7）：回复区仅渲染散文。`clarificationReplyText()` 在**两处**回复区写入点（流式前导帧 `renderStreamingReply` 与回合收尾 `applyOutcome` 的 clarification 分支）统一调用 `classifyInlineEditClarification`（InlineEditPrompt）：工具调用流（`<|tool call>` 系、antml `<function_calls>`、`<tool_call>` 系等）或协议标签片段返回诚实提示文案（`inlineEdit.reply.toolCallInspectedContext` / `inlineEdit.reply.unrenderableProtocolOutput`），绝不渲染内部标记；fail-closed——无预览、无写入，回合以可读消息呈现（会话存活，用户可改写或移除附件后重试）
