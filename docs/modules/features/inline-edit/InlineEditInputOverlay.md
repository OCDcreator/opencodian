...
- DOM 布局契约（Cursor cmd-K 指令条骨架，详见 `docs/modules/style/features/inline-edit.md`）：澄清/错误块（图标 + 文本）置顶 → `inputrow`（sparkles 引导图标 + 无边框输入框 + 实心提交/幽灵关闭按钮）→ `chipbar` 页脚（模型 chip = 提供商品牌图标 + 模型名；努力 chip = brain 图标 + 可见文字标签 + 值；无 kbd 提示）。chip 前缀图标经 `createProviderIcon` 回调（与主输入窗口同一 `ProviderIconService` 管线）解析，失败回退 lucide 字形；chip 的 `title` 提供完整标签 tooltip。菜单行统一 13px 图标槽（模型行品牌图标、努力行 signal-low/medium/high 信号格、清除行 messages-square/rotate-ccw），保证 label 对齐；busy 时提交按钮图标换 `loader-circle` 并加 `.opencodian-inline-edit-spinning`，引导图标 pulse
