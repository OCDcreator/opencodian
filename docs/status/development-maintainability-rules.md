...
- `.opencodian-container` carries `container-type: inline-size` (R-F5), so it is a containing block for `position: fixed` descendants: a fixed child would be positioned against that container instead of the viewport. **This is a registered latent constraint, not a fixed bug** — as of 2026-09-21 the plugin owns no fixed-position descendant of that container. Every fixed overlay is mounted at `document.body` (`.opencodian-tooltip-layer` in `shared/TooltipLayerController.ts`, `.opencodian-image-preview-backdrop` in `features/chat/ui/ImagePreviewOverlay.ts`, `.opencodian-tab-overflow-menu` in `features/chat/tabs/TabBar.ts`, the portaled `.opencodian-settings-dropdown-menu`) or lives in the settings window (`.opencodian-agent-switcher-floating`). Before adding a fixed-position element inside the chat container, either mount it at `document.body` (the established pattern) or move the container-query anchor deliberately — do not assume viewport coordinates. Do not "narrow" the anchor casually: `@container (min-width: 640px)` in `src/style/base/core.css` is the only unnamed container query, it drives the R-F5 session rail's visibility breakpoint, and re-anchoring it to the messages shell would change when the rail appears while moving the containment hazard one level down into a subtree that has more descendants.
