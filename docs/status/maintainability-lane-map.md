...
> **当前状态**: [READY] `R48` model section owner seam 已完成；当前首个 `[NEXT]` 为 `R49` style section lifecycle seam。

## 当前优先级

- **当前 `[NEXT]`**：`R49 - OpenCodianSettings style section lifecycle seam`
- **本批目标**：沿既定顺序完成 `OpenCodianSettings` style 厚切口，然后进入 checkpoint
- **当前 lint 基线**：`0 errors / 90 warnings`
- **热点顺序**：
  1. `src/features/settings/OpenCodianSettings.ts` style section lifecycle seam
  2. `R50` checkpoint

## 本批边界

- `R48` 已完成 `OpenCodianSettings` model section owner seam；后续 queue 不允许把 source mode / refresh / workspace / icon cache lifecycle 搬回主类
