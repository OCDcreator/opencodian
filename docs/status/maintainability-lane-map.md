...
> **当前状态**: [READY] `R45` 已完成；当前首个 `[NEXT]` 为 `R46`。

## 当前优先级

- **当前 `[NEXT]`**：`R46 - Maintainability checkpoint`
- **本批目标**：复盘 `R42-R45` 的 owner 收益、验证成本与下一批优先切口
- **当前 lint 基线**：`0 errors / 86 warnings`
- **热点顺序**：
  1. `R46` checkpoint / next-batch decision

## 本批边界

- `R42-R45` 已完成 history/actions、authoritative sync merge、model catalog/selection 与 streaming transport seam；本轮后直接顺延到 `R46`
...
- `OpenCodeService`：SDK-first / legacy fallback、session-scoped abort/detach、stream finalization 语义不变；checkpoint 后再决定是否继续 settings reconfiguration seam
