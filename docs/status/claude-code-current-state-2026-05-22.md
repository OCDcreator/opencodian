...
## 2026-06-08 Round 18 — MCP Elicitation SDK Audit (0.3.145 vs 0.3.168)

### Objective

Determine whether the latest official `@anthropic-ai/claude-agent-sdk` (npm 0.3.168) changes the MCP Elicitation / `onElicitation` behavior enough to achieve a real diagnostic live pass or product-path proof.

### Audit method

1. **Type inspection**: `diff -u` of `sdk.d.ts` between local installed 0.3.145 and npm latest 0.3.168.
2. **Isolated runtime reproduction**: Standalone Node.js harness using the same temp MCP stdio server code and prompt strategy as `ClaudeCodeAdapter.runMcpElicitationLiveProbe()`, run against both SDK versions in clean temp directories with platform-specific binary packages.
3. **Source inspection**: `sdk.mjs` grep for `onElicitation` / `elicitation` references; `package.json` peer dependency comparison.

### Findings

- **Type signatures unchanged**: `onElicitation`, `ElicitationRequest`, `ElicitationResult`, `mcpServers`, `allowedTools`, and `disallowedTools` types are identical between 0.3.145 and 0.3.168. `OnElicitation` callback signature: `(request: ElicitationRequest, options: { signal: AbortSignal }) => Promise<ElicitationResult>`.
...
- **Isolated runtime results**:
  - **SDK 0.3.145**: PASS — `onElicitation` invoked 1 time (`serverName: 'elicit_smoke'`, `mode: 'form'`), tool result echoed `elicitationAction: 'accept'`, full chain verified.
  - **SDK 0.3.168**: PASS — identical behavior, `onElicitation` invoked 1 time, full chain verified.

### Conclusion

**The SDK is NOT the blocker.** Both 0.3.145 and 0.3.168 correctly invoke `onElicitation` when queried directly with `mcpServers` + `onElicitation` + `allowedTools`. The Test Vault live probe failure (BUILD_IDs `feature-phase0-capability.202606080238` and `feature-phase0-capability.202606080313`) is in the adapter/integration layer — specifically how `runDiagnosticPrompt()` builds and passes options, or the Test Vault runtime environment — not in the SDK itself.
...
## 2026-06-08 Round 16 — MCP Elicitation Diagnostic Live Probe

### Objective

Upgrade the existing synthetic `Probe MCP Elicitation Wiring` with a live diagnostic probe that attempts the actual SDK `onElicitation` path with a temporary MCP stdio server, while maintaining honest product-path boundaries.

### What changed

1. **New adapter method**: `ClaudeCodeAdapter.runMcpElicitationLiveProbe()` creates a temp MCP stdio server with `elicitation` capability, runs a diagnostic prompt via `runDiagnosticPrompt()` with `_diagnosticMcpServers` + `_diagnosticAllowedTools` + `_diagnosticCanUseTool` + `_diagnosticOnElicitation` overrides, and attempts the full chain:
...
|--------|----------|--------|
| **npm `@anthropic-ai/claude-agent-sdk`** | Latest version: **0.3.168** (Round 18 audit) | Installed: 0.3.145 |
| **CLI `@anthropic-ai/claude-code`** | Latest version: **2.1.168** | — |
...
Installed SDK: 0.3.145. Latest npm: 0.3.168 (Round 18 audit). No changelog evidence of SessionStore graduation from alpha.
...
2. **No changelog fix through 0.3.168** (Round 18 audit): 23 versions between 0.3.145 and 0.3.168 contain zero mentions of checkpointing fixes.
...
- Current-state doc: updated readback entry with 2026-06-07 evidence (latest npm 0.3.168 as of Round 18 audit; prior audit against 0.3.167, closed issues, docs discrepancy).
...
> **Superseded current-state note (2026-06-08):** this historical 24-row summary predates the `AskUserQuestion` / `MCP Elicitation` split. The authoritative current matrix is the top-of-file 55-row audit: `AskUserQuestion` remains `pass` through the built-in tool path, while `MCP Elicitation` is a separate `wiring` row. **Round 16 (2026-06-08) added a diagnostic live probe**: a temporary MCP stdio server is created and the probe attempts to verify the full chain (`elicitation/create` → `onElicitation` → callback response → server consumption → nonce echo). **Test Vault validation has NOT achieved `pass`**: in BUILD_ID `feature-phase0-capability.202606080238` the model called the tool but `onElicitation` was not invoked; in BUILD_ID `feature-phase0-capability.202606080313` the model did not call the tool at all. The probe is useful for diagnosis but does not yet prove a stable SDK/diagnostic real MCP roundtrip. Stable `pass` still requires either (a) Test Vault evidence that the SDK invokes `onElicitation`, or (b) actual Obsidian shared question-dialog product-path proof with a real user's answer consumed by the server.
