...
## Structured Output Prompt Hardening (2026-05-27)

A focused gap-closing pass addressed the remaining ordinary `/json` chat UX issue: the model could emit extra prose or markdown JSON code fences before or alongside the StructuredOutput tool call.

### Gap Found

In the ordinary `/json` chat path, after duplicate suppression and hook text fixes were proven, the model could still emit visible prose such as "All done! Said hello in JSON." or markdown code-fenced JSON before calling the StructuredOutput tool. This prose was not a duplicate of the structured output itself, but rather extraneous text generated by the model outside the tool call.

### What Changed

**Plugin-side prompt hardening** in `src/core/agents/backend/ClaudeCodeAdapter.ts`:
- `sendMessage()` now detects when `request.options.outputFormat` is present (the signal for a structured-output send)
- When detected, the user prompt is prefixed with: `Respond ONLY using the required structured output format. Do not include markdown code blocks, JSON fences, or additional prose.`
- This hardening applies only to structured-output sends; ordinary chat messages are unaffected
- The hardening is intentionally minimal and honest: it reduces the probability of extra prose without guaranteeing complete elimination, which would require SDK/model-side constraints

**Tests**: 2 new tests in `tests/unit/core/agents/backend/ClaudeCodeAdapter.test.ts`:
- `prefixes structured-output sends with a hardening prompt constraint` — verifies the hardened prompt content reaches the SDK
- `does not harden prompt for sends without outputFormat` — verifies ordinary sends remain unchanged

### Runtime Proof

**BUILD_ID**: `feature-phase0-capability.202605272354`

**Test Vault verification** (obsidian-plugin-autodebug):
- First `/json` message (`/json say hello in JSON`): assistant message contains 0 text blocks, structured output badge present, no hook text
- Second `/json` message (`/json generate a greeting`): assistant message contains 0 text blocks, structured output badge present, no hook text
- Reload/hydration verification: persisted conversation survives reload with structured output intact, 0 text blocks, no regression
- Console: no errors
- Model thinking blocks explicitly reference the StructuredOutput tool constraint, confirming the hardening prompt reaches the model

**Classification**: **A — Plugin hardening successful**
- Assistant messages contain only structured output (plus thinking summary, which is normal SDK behavior)
- No extra prose, no markdown JSON code fences, no duplicate raw JSON text blocks
- Previous gap (extra prose like "All done!") is no longer reproducible with hardening enabled

### Remaining Boundary

The hardening prompt reduces but does not mathematically guarantee zero extra prose. If future model behavior changes or edge cases emerge, the remaining prose would be classified as an SDK/model boundary issue, not a plugin gap. The plugin has applied the most reasonable client-side constraint available.

**Artifacts**:
- `.obsidian-debug/json-hardening-proof-20260527.png` — screenshot of first `/json` result
- `.obsidian-debug/json-hardening-second-msg-20260527.png` — screenshot of second `/json` result
- `.obsidian-debug/json-hardening-console-20260527.txt` — console capture
- `.obsidian-debug/json-hardening-errors-20260527.txt` — errors capture ("No errors captured.")
- `.obsidian-debug/json-hardening-proof-result-20260527.json` — structured DOM assertion result
