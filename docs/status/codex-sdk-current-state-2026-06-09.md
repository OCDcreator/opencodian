...
> **2026-07-26 P3 final acceptance**: the repository remains pinned to `@openai/codex-sdk`/bundled Codex `0.139.0`, while the actually installed CLI is `codex-cli 0.144.1`; do not treat them as one version. (1) `hooks/list` is `{cwds?: string[]}` → `{data:[{cwd,hooks,warnings,errors}]}`, readonly readback-only with no `hooks/*` write route; Test Vault QA observed non-empty Hooks UI and passed the full state matrix, zh/en dark/light, and 314px modal width (346px viewport equivalent). (2) `thread/compact/start` takes `{threadId}` and returns an empty ACK; runtime success requires accepted ACK, a post-dispatch same nonempty item ID `item/started` → matching `item/completed` for `contextCompaction`, and fresh `thread/tokenUsage/updated`; completion-only, replayed, or mismatched IDs are not verified. It is a non-config runtime action; real success and confirmation/pending/duplicate/reject/no-thread/session-switch stale-fencing/timeout-warning QA passed. Test Vault allows one tab, so the real stale case was an active-tab conversation/session switch; tab-ID switching is automated-test coverage. (3) legacy active `profile = "name"` is rejected in 0.144.1; the supported mechanism is `$CODEX_HOME/<name>.config.toml` + CLI `--profile`. App-server/thread routes have no profile-selection parameter or verified persistent active-profile field. G16 remains `BLOCKED / CURRENT CONTRACT CHANGED`; no config write or editor/switcher was implemented. Evidence: `/Users/dht/.codex/visualizations/2026/07/26/019f9df6-ec92-7f80-8ed9-c282c6900bde/p3-qa-main.202607262020/019f9df6-ec92-7f80-8ed9-c282c6900bde-manual-qa.md`.
...
| `thread/compact/start` | `runtime verified / NON-CONFIG` | P3 runtime action: `{threadId}` → empty ACK only. Runtime verified requires accepted ACK, post-dispatch same nonempty item ID `item/started` → matching `item/completed` for `contextCompaction`, and fresh `thread/tokenUsage/updated`; completion-only/replayed/mismatched IDs do not verify. Real successful compaction and confirmation/pending/duplicate/reject/no-thread/session-switch stale-fencing/timeout-warning Test Vault QA passed; the real stale case was an active-tab conversation/session switch because Test Vault permits one tab. |
...
| `hooks/list` | `readback` | P3 settings readback: `{cwds?: string[]}` → `{data:[{cwd,hooks,warnings,errors}]}`. No hooks write route was found, so this is not CRUD/config editing. Real non-empty Hooks UI, full state matrix, zh/en dark/light, and 314px modal-width Test Vault QA passed. |
| Codex config profiles | `blocked / current contract changed` | 0.144.1 rejects legacy active `profile = "name"`; select V2 `$CODEX_HOME/<name>.config.toml` only via CLI `--profile`. App-server/thread routes expose no profile selector or verified persistent active-profile field; do not conflate with `permissionProfile/list`. |
...
> **Note**: This is the **checkpoint 0 baseline** matrix. Statuses here reflect the state BEFORE any implementation. For the current (latest) classification, use **§1 Executive Summary** plus the reviewed checkpoint artifacts such as `docs/archive/maintainability/phases/checkpoint-7b-codex-todo-completion-refresh.md`.
...
| `docs/archive/maintainability/phases/checkpoint-8c-mcp-transcript-seam-truth-productization.md` | Created (this checkpoint artifact) |
...
See `docs/archive/maintainability/phases/checkpoint-10a-codex-runtime-settings-truth-split.md`.
...
See `docs/archive/maintainability/phases/checkpoint-10b-websearchmode-cached-vs-live-audit.md`.
...
See `docs/archive/maintainability/phases/checkpoint-10c-websearchmode-sdk-cli-semantics-audit.md`.
...
See `docs/archive/maintainability/phases/checkpoint-11a-codex-image-input-seam-audit.md`.
...
See `docs/archive/maintainability/phases/checkpoint-11b-codex-image-input-composer-adapter-slice.md`.
...
See `docs/archive/maintainability/phases/checkpoint-12a-codex-backend-session-browser-audit.md`.
...
See `docs/archive/maintainability/phases/checkpoint-12b-codex-persisted-conversation-resume-audit.md`.
...
See `docs/archive/maintainability/phases/checkpoint-11c-codex-image-input-paste-dragdrop.md`.
...
See `docs/archive/maintainability/phases/checkpoint-12c-codex-provisional-warning.md`.
