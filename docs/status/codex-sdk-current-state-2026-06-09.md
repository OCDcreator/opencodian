...
> **Last updated**: 2026-06-12 (Checkpoint 15H corrected: **Capability Lab Codex matrix shrunk from 15 per-capability rows to 4 category-summary rows** — one per truth bucket (`pass`, `settings-only`, `readback`, `blocked`). No umbrella row spans mixed states. Surface labels corrected: Permission Profiles / Rate Limits are `settings` readbacks, not `diagnostic`. BUILD_ID `feature-codex-sdk-capability.202606120111`, 273 capability-lab tests pass. Capability Lab remains diagnostic — Codex rows are status summary, not stable settings surface.)
