...
> **Audit scope**: Checkpoint 0–15E (capability matrix → SDK smoke → adapter skeleton → hidden/runtime surfaces → ordinary transcript/product seams → structured output schema fix → runtime settings truth split → `webSearchMode` cached-vs-live runtime audit → SDK/CLI semantics audit → image input seam audit → image input composer/adapter slice → backend session browser truth audit → persisted conversation resume audit → image paste/drag-drop polish → `webSearchMode` truth resolution → global sandboxMode settings productization → global modelReasoningEffort settings productization → approval-policy truth split → app-server feasibility audit → local app-server surface audit → local app-server protocol/shape audit → app-server surface mapping audit → app-server session discovery / transcript readback integration → Codex active-backend settings readback seams for account info / model list / permission profiles / rate limits / account usage)
> **Last updated**: 2026-06-11 (Checkpoint 15E final truth sync. The earlier request-shape repair for `account/usage/read` was real, but the stronger root cause is now proven from the exact Test Vault bundled binary: `codex-cli 0.137.0` generates an app-server `ClientRequest` union that does **not** include `account/usage/read`. To avoid exposing a dead control, 15E is now **hidden from the ordinary active-backend settings surface**, while the underlying capability remains **`blocked`** on the current bundled runtime. Fresh focused tests pass (`CodexAppServerClient.accountUsage` 5/5, `CodexAppServerClient.rateLimits` 3/3, `SettingsCodexSection.accountUsage` 1/1, `SettingsCodexSection` 14/14), Test Vault reload is on BUILD_ID `feature-codex-sdk-capability.202606112222`, and `dev:errors` stays clean.)
