...
> **Audit scope**: Checkpoint 0–15I (capability matrix → SDK smoke → adapter skeleton → hidden/runtime surfaces → ordinary transcript/product seams → structured output schema fix → runtime settings truth split → `webSearchMode` cached-vs-live runtime audit → SDK/CLI semantics audit → image input seam audit → image input composer/adapter slice → backend session browser truth audit → persisted conversation resume audit → image paste/drag-drop polish → `webSearchMode` truth resolution → global sandboxMode settings productization → global modelReasoningEffort settings productization → approval-policy truth split → app-server feasibility audit → local app-server surface audit → local app-server protocol/shape audit → app-server surface mapping audit → app-server session discovery / transcript readback integration → Codex active-backend settings readback seams for account info / model list / permission profiles / rate limits / account usage → global webSearchMode settings productization → session-level webSearchMode override → Codex structured reply chip / `/json` / `outputSchema` truth-sync)
> **Last updated**: 2026-06-12 (Checkpoint 15I truth-sync: **Codex structured reply chip / `/json` / `outputSchema` audit** confirmed code is already fully wired — no product gap. Composer capability hint, send pipeline `/json` trigger, CodexAdapter outputSchema forwarding, CodexStreamNormalizer structured_output promotion, and shared badge rendering all work for both Claude Code and Codex. Truth-sync applied to 8 docs/module files that incorrectly said "Claude-only" for these surfaces. **Evidence boundary**: hint visible for Codex in runtime UI (DOM + screenshot proof on active backend `codex`), code path wired, unit-tested. Full live end-to-end Codex structured output turn (actual `/json` send → badge render → persisted conversation) not freshly captured in this session. BUILD_ID `feature-codex-sdk-capability.202606121257`.)
