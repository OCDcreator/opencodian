...
> **Audit scope**: Checkpoint 0–15K (capability matrix → SDK smoke → adapter skeleton → hidden/runtime surfaces → ordinary transcript/product seams → structured output schema fix → runtime settings truth split → `webSearchMode` cached-vs-live runtime audit → SDK/CLI semantics audit → image input seam audit → image input composer/adapter slice → backend session browser truth audit → persisted conversation resume audit → image paste/drag-drop polish → `webSearchMode` truth resolution → global sandboxMode settings productization → global modelReasoningEffort settings productization → approval-policy truth split → app-server feasibility audit → local app-server surface audit → local app-server protocol/shape audit → app-server surface mapping audit → app-server session discovery / transcript readback integration → Codex active-backend settings readback seams for account info / model list / permission profiles / rate limits / account usage → global webSearchMode settings productization → session-level webSearchMode override → Codex structured reply chip / `/json` / `outputSchema` truth-sync → `webSearchMode` bundled-runtime audit)
> **Last updated**: 2026-06-12 (Checkpoint 15K: **Codex `webSearchMode` bundled-runtime audit**. Using the exact Test Vault/plugin `codex-cli 0.137.0` binary with isolated alternate web-access paths disabled, `--config web_search="disabled"` produced 0 built-in `web_search` tool calls, while `--config web_search="cached"` produced 32 and `--config web_search="live"` produced 2. This proves a runtime `disabled` vs `enabled` (`cached`/`live`) boundary. No observable transcript-level distinction between `cached` and `live` was found. Classification remains **`settings-only`**; no code changes. Durable evidence at `testvault/.obsidian-debug/15k-bundled-websearchmode-*`.)
...
- **settings-only**
  - **global ordinary settings surface `webSearchMode`** (Checkpoint 15F: dropdown with `disabled`/`cached`/`live` in ordinary active-backend settings; persisted to `CodexBackendSettings`; live adapter writeback via `updateWebSearchMode()`; honest next-thread boundary copy. **Checkpoint 15K**: bundled-runtime proof using the exact Test Vault/plugin `codex-cli 0.137.0` binary with alternate web-access paths disabled (`--disable browser_use browser_use_external computer_use plugins hooks shell_tool`). `--config web_search="disabled"` produced **0** built-in `web_search` tool calls; `--config web_search="cached"` produced **32**; `--config web_search="live"` produced **2**. This proves the `disabled` vs `enabled` (`cached`/`live`) runtime boundary. **Honest classification**: remains `settings-only` because no observable transcript-level distinction between `cached` and `live` was found in the bundled runtime; the full ternary distinction advertised by the UI is therefore only partially proven at runtime.)
  - **session modal per-conversation `webSearchMode`** (Checkpoint 15G: four-option dropdown inherit/disabled/cached/live in `ConversationSessionSettingsModal`; persisted to `ConversationSessionSettings.codexWebSearchMode`; coordinator resolve/save/apply forwards to `updateWebSearchMode()`. **Checkpoint 15K**: same bundled-runtime evidence as global `webSearchMode` applies here because the coordinator forwards the resolved mode through the same `updateWebSearchMode()` path and the SDK emits the same `--config web_search="<mode>"` CLI flag. **Honest classification**: remains `settings-only` because the `cached`/`live` mutual distinction is not observable in the bundled runtime output.)
- **readback**
...
- **已 pass** (Checkpoint 9E, truth-synced 2026-06-12)
