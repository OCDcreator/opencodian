...
> **Last updated**: 2026-06-12 (Checkpoint 15J: **Codex per-conversation `networkAccessEnabled` runtime divergence promoted from `readback` to `已 pass`**. Bundled-runtime proof using the exact Test Vault/plugin `codex-cli 0.137.0` binary: `--config sandbox_workspace_write.network_access=false` → curl exit code 6 (DNS blocked) vs `=true` → curl exit code 0 (HTTP 200, remote IP `198.18.0.193` returned). Plugin SDK generates the exact same CLI flag. UI/persistence/plumbing proven in 13C. Durable evidence at `testvault/.obsidian-debug/15j-bundled-*`. No code changes. Docs-only truth-sync.)
