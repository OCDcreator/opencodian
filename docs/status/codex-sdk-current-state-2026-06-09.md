...
> **Audit scope**: Checkpoint 0–15F (capability matrix → SDK smoke → adapter skeleton → hidden/runtime surfaces → ordinary transcript/product seams → structured output schema fix → runtime settings truth split → `webSearchMode` cached-vs-live runtime audit → SDK/CLI semantics audit → image input seam audit → image input composer/adapter slice → backend session browser truth audit → persisted conversation resume audit → image paste/drag-drop polish → `webSearchMode` truth resolution → global sandboxMode settings productization → global modelReasoningEffort settings productization → approval-policy truth split → app-server feasibility audit → local app-server surface audit → local app-server protocol/shape audit → app-server surface mapping audit → app-server session discovery / transcript readback integration → Codex active-backend settings readback seams for account info / model list / permission profiles / rate limits / account usage → global webSearchMode settings productization)
> **Last updated**: 2026-06-11 (Checkpoint 15F correction: `webSearchMode` settings surface productized — **settings-only**. Dropdown with `disabled`/`cached`/`live` options rendered in `SettingsCodexSection`; persisted to `CodexBackendSettings`; live adapter update via `updateWebSearchMode()` for next-thread boundary. The type, normalization, adapter wiring, and locale strings all pre-existed; only the UI control and runtime update method were missing. **Honesty note**: settings persistence and adapter option wiring are verified, but distinct runtime web-search behavior between `disabled`/`cached`/`live` modes has **not** been end-to-end proven in this round. The capability is classified `settings-only`, not `已 pass`. Targeted tests pass (`CodexAdapter.updateWebSearchMode` 2/2, `SettingsCodexSection.webSearchMode` 4/4 including DOM render + persistence + adapter call + exact-count regression), full verify green (496 suites, 4683 tests), Test Vault reload on BUILD_ID `feature-codex-sdk-capability.202606120007`, `dev:errors` stays clean. Contracted stable settings surface is now `apiKey + model + sandboxMode + modelReasoningEffort + additionalDirectories + networkAccessEnabled + webSearchMode`.)
