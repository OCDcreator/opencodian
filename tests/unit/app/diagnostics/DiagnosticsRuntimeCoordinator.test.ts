...
describe('Phase 3 Task 11 — DiagnosticsRuntimeCoordinator (behavior)', () => {
  ...
  describe('construction wires each backend its own options', () => {
    ...
    it('each backend redacts its OWN knownSecrets (no cross-backend leak)', async () => {
      // If options were swapped, the OpenCode redactor (static snapshot) would
      // redact the wrong secret. Pin that each backend redacts its own.
      const inputs = makeInputs();
      const coord = new DiagnosticsRuntimeCoordinator(inputs);
      created.push(coord);
      ...
      // codex-secret/claude-secret are NOT in OpenCode's knownSecrets → would leak.
      expect(ocBlob).not.toContain('codex-secret');
    });
  ...
  describe('dispose', () => {
    ...
    it('dispose does not leak Codex trace disposal error secrets in warnings (injected logger)', async () => {
      ...
      await coord.dispose();
      // The fire-and-forget catch runs in a microtask; flush it.
