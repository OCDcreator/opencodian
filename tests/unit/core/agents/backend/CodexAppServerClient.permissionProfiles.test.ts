/**
 * Real unit tests for CodexAppServerClient.listPermissionProfiles().
 *
...
describe('CodexAppServerClient.listPermissionProfiles', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('returns profiles from app-server response', async () => {
    const proc = createMockProcess();
    mockSpawn.mockReturnValue(proc);
    emitWsUrl(proc.stdout!);

    // Resolve the WebSocket open promise
    setTimeout(() => mockWsInstance.onopen?.(), 10);

    const client = new CodexAppServerClient({ codexPathOverride: '/path/to/codex' });

    // We need to intercept the JSON-RPC requests to simulate responses.
    // Track request IDs and respond accordingly.
    let initRequestId = 0;
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      if (msg.method === 'initialize') {
        ...
        setTimeout(() => simulateResponse(initRequestId, {}), 5);
    ...
    const profiles = await client.listPermissionProfiles();
  ...
  it('passes cwd option to the request', async () => {
    const proc = createMockProcess();
    mockSpawn.mockReturnValue(proc);
    emitWsUrl(proc.stdout!);
    ...
    await client.listPermissionProfiles({ cwd: '/some/path' });

    expect(capturedParams).toMatchObject({ cwd: '/some/path' });
  });
  ...
  it('passes limit and cursor options to the request', async () => {
    ...
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      if (msg.method === 'initialize') {
        ...
        capturedParams = msg.params;
        setTimeout(() => {
          simulateResponse(msg.id, { data: [{ id: 'next' }] });
        }, 5);
      }
    });

    await client.listPermissionProfiles({ limit: 10, cursor: 'abc123' });
