...
describe('CodexAppServerClient mcpServerToolCall', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockWsInstance.send.mockClear();
  ...
  it('resumes the thread then calls mcpServer/tool/call with { threadId, server, tool, arguments }', async () => {
    ...
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      if (msg.method === 'thread/resume') {
        callOrder.push('thread/resume');
        setTimeout(() => simulateResponse(msg.id, { thread: { id: msg.params.threadId } }), 5);
      }
      if (msg.method === 'mcpServer/tool/call') {
        callOrder.push('mcpServer/tool/call');
    ...
    const result = await client.mcpServerToolCall('thread-1', 'auth_test', 'fetch_secure_data', { query: 'hello' });

    expect(callOrder).toEqual(['thread/resume', 'mcpServer/tool/call']);

    const toolCallSent = mockWsInstance.send.mock.calls
      .map((c) => JSON.parse(c[0] as string))
      .find((m) => m.method === 'mcpServer/tool/call');
    expect(toolCallSent.params).toEqual({
      threadId: 'thread-1',
      server: 'auth_test',
  ...
  it('returns isError=true when the MCP server reports an error', async () => {
    const client = await createInitializedClient();
    ...
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      if (msg.method === 'mcpServer/tool/call') {
        setTimeout(() => simulateResponse(msg.id, {
          content: [{ type: 'text', text: 'Error: still broken' }],
          isError: true,
        }), 5);
      }
    });

    const result = await client.mcpServerToolCall('thread-1', 'srv', 'tool', {});

    expect(result).not.toBeNull();
    expect(result!.isError).toBe(true);
    expect(result!.content[0].text).toBe('Error: still broken');
  });

  it('returns errorReason when the app-server returns a JSON-RPC error', async () => {
    const client = await createInitializedClient();
    ...
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      if (msg.method === 'mcpServer/tool/call') {
        setTimeout(() => simulateError(msg.id, { code: -32600, message: 'missing field threadId' }), 5);
      }
    });
    ...
    expect(result!.errorReason).toContain('missing field threadId');
  });

  it('returns empty content when app-server returns no content array', async () => {
    const client = await createInitializedClient();
