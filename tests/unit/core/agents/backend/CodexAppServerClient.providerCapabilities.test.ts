/**
 * Real unit tests for CodexAppServerClient.getModelProviderCapabilities().
 *
...
describe('CodexAppServerClient.getModelProviderCapabilities', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('returns capabilities from app-server response', async () => {
    ...
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      } else if (msg.method === 'modelProvider/capabilities/read') {
        setTimeout(() => {
    ...
    const capabilities = await client.getModelProviderCapabilities();
  ...
  it('returns null when response lacks capability fields', async () => {
    const proc = createMockProcess();
    mockSpawn.mockReturnValue(proc);
    emitWsUrl(proc.stdout!);
    ...
    mockWsInstance.send.mockImplementation((data: string) => {
      ...
      if (msg.method === 'initialize') {
        ...
        setTimeout(() => {
          simulateResponse(msg.id, { someOtherField: true });
        }, 5);
      }
    });
  ...
  it('returns partial capabilities when only some flags are present', async () => {
    const proc = createMockProcess();
    mockSpawn.mockReturnValue(proc);
    emitWsUrl(proc.stdout!);
