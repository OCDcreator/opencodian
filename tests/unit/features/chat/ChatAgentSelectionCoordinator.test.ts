...
describe('ChatAgentSelectionCoordinator', () => {
  ...
  it('loads primary-agent candidates, selects one, and restores composer focus', async () => {
    ...
    expect(fixture.container.querySelector<HTMLElement>('.opencodian-agent-dropdown-state')?.textContent).toBe('Loading agents...');
    await settleAsyncWork();
    const option = fixture.container.querySelector<HTMLElement>('[data-agent-id="build"]');
    option?.click();

    expect(fixture.coordinator.getSelectedAgentId()).toBe('build'); expect(fixture.coordinator.isOpen()).toBe(false);
    expect(trigger?.textContent).toBe('Build'); expect(trigger?.hasClass('is-selected')).toBe(true);
  ...
  it('clamps the agent dropdown to the chat boundary when it opens', () => {
    ...
    expect(dropdown?.style.left).toBe('-12px'); expect(dropdown?.style.width).toBe('204px');
  ...
  it('disconnects the previous boundary observer when remounted', () => {
    ...
    expect(firstObserver?.disconnect).toHaveBeenCalledTimes(1); expect(ResizeObserverMock.instances).toHaveLength(2);
  ...
  it('renders the dropdown as an accessible compact agent list without row detail toggles', async () => {
    ...
    expect(defaultOption?.querySelector<HTMLElement>('.opencodian-agent-option-mode.is-default-mode')?.textContent).toBe('Project default');
    expect(defaultOption?.querySelector<HTMLElement>('.opencodian-agent-option-desc')?.textContent).toBe('Let OpenCode choose the project default primary agent.');
    ...
    expect(trigger?.getAttribute('aria-expanded')).toBe('false');
    expect(fixture.container.querySelector<HTMLElement>('[data-agent-id="build"]')?.getAttribute('aria-selected')).toBe('true');
  });
  ...
  describe('keyboard navigation', () => {
    it('focuses the selected default agent after keyboard opening settles', async () => {
      const fixture = createFixture(); const trigger = fixture.container.querySelector<HTMLElement>('.opencodian-agent-trigger');
      trigger?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
      await settleAsyncWork();
      const defaultOption = fixture.container.querySelector<HTMLElement>('[data-agent-id=""]'); const options = fixture.container.querySelectorAll<HTMLElement>('.opencodian-agent-option');
      expect(document.activeElement).toBe(defaultOption); expect(defaultOption?.tabIndex).toBe(0);
    ...
    it('wraps roving agent focus in both directions', async () => {
      const fixture = createFixture(); const trigger = fixture.container.querySelector<HTMLElement>('.opencodian-agent-trigger');
      trigger?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); await settleAsyncWork();
      const dropdown = fixture.container.querySelector<HTMLElement>('.opencodian-agent-dropdown');
    ...
    it('selects the focused agent with Enter and restores composer focus once', async () => {
      ...
      trigger?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); await settleAsyncWork();
      const dropdown = fixture.container.querySelector<HTMLElement>('.opencodian-agent-dropdown');
      dropdown?.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true })); dropdown?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
      expect(fixture.coordinator.getSelectedAgentId()).toBe('build'); expect(fixture.coordinator.isOpen()).toBe(false);
      expect(fixture.host.restoreInputFocus).toHaveBeenCalledTimes(1);
    });
    ...
    it('closes from list Escape and returns focus to the trigger', async () => {
      ...
      expect(fixture.coordinator.isOpen()).toBe(false); expect(document.activeElement).toBe(trigger);
      expect(
    ...
    it.each([
      ...
      const fixture = createFixture(); fixture.host.loadAgentSelectionCandidates.mockImplementation(loadCandidates);
      const trigger = fixture.container.querySelector<HTMLElement>('.opencodian-agent-trigger');
      trigger?.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
      await settleAsyncWork();
      const stateEl = fixture.container.querySelector<HTMLElement>('.opencodian-agent-dropdown-state');
      expect(fixture.coordinator.isOpen()).toBe(true); expect(document.activeElement).not.toBe(stateEl);
      expect(stateEl?.tabIndex).toBe(-1); fixture.coordinator.closeDropdown();
