...
describe('SettingsTabbedRenderer', () => {
  ...
  it('filters OpenCode-only conversation secondary tabs when OpenCode is disabled', () => {
    ...
    expect(secondaryTabs).toHaveLength(6);
    expect(Array.from(secondaryTabs).map((tab) => tab.dataset.tabId)).toEqual(['title', 'display', 'memory', 'vault-retrieval', 'inline-edit', 'context-groups']);
