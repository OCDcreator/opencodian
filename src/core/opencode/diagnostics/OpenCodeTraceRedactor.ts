...
export interface OpenCodeTraceRedactorOptions {
  vaultPath?: string;
...
export interface OpenCodeRedactionStats {
...
export interface OpenCodeRedactionResult<T = unknown> {
  value: T;
  stats: OpenCodeRedactionStats;
}
...
export class OpenCodeTraceRedactor {
  private readonly options: Required<Omit<OpenCodeTraceRedactorOptions, 'vaultPath' | 'diagnosticsPath' | 'temporaryPath' | 'knownSecrets'>>
    & Pick<OpenCodeTraceRedactorOptions, 'vaultPath' | 'diagnosticsPath' | 'temporaryPath' | 'knownSecrets'>;

  constructor(options: OpenCodeTraceRedactorOptions = {}) {
    this.options = {
      ...options,
  ...
  redact<T>(value: T, kind: 'ordinary' | 'stack' | 'service-output' = 'ordinary'): OpenCodeRedactionResult<T> {
    const stats: OpenCodeRedactionStats = {
      secretsRemoved: 0,
      pathsNormalized: 0,
      valuesTruncated: 0,
  ...
  private redactEnvironmentObject(value: object, stats: OpenCodeRedactionStats): Record<string, string> {
