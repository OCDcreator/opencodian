...
interface PersistedTraceIndex {
  ...
  summaries: Record<string, OpenCodeTraceSummary>;
}

function defaultUserDataDirectory(): string {
  const dynamicRequire = (globalThis as typeof globalThis & { require?: (id: string) => unknown }).require;
  try {
    const remote = dynamicRequire?.('@electron/remote') as { app?: { getPath?: (name: string) => string } } | undefined;
    const userData = remote?.app?.getPath?.('userData');
    if (userData) return userData;
  } catch {
    // Electron remote is optional.
  }
...
export function resolveDefaultOpenCodeTraceDirectory(): string {
  return path.join(defaultUserDataDirectory(), 'OpenCodian', 'diagnostics', 'opencode');
}

export class OpenCodeTraceStore {
  ...
  private memoryEvents: OpenCodeTraceEventV1[] = [];
  ...
  private pressureNoticeTemplate: OpenCodeTraceEventV1 | undefined;
  private degradedListener: ((
    error: unknown,
    template?: OpenCodeTraceEventV1,
  ) => void) | undefined;
  ...
  constructor(
    ...
    private readonly fallbackDirectory = resolveDefaultOpenCodeTraceDirectory(),
  ) {
    this.requestedDirectory = customDirectory?.trim() || resolveDefaultOpenCodeTraceDirectory();
    this.rootDirectory = this.requestedDirectory;
    this.initialized = this.initialize();
  }

  append(event: OpenCodeTraceEventV1, deep = false): void {
    let structuralEvent: OpenCodeTraceEventV1;
    let json: string;
  ...
  private enqueueNotice(event: OpenCodeTraceEventV1, name: string, count: number): boolean {
    const notice: OpenCodeTraceEventV1 = {
      ...event,
      severity: 'warning',
      name,
      payload: { count },
  ...
  private canCoalesce(event: OpenCodeTraceEventV1): boolean {
  ...
  onDegraded(listener: (error: unknown, template?: OpenCodeTraceEventV1) => void): void {
  ...
  listSummaries(limit = 20): OpenCodeTraceSummary[] {
  ...
  async readTrace(traceId: string): Promise<OpenCodeTraceEventV1[]> {
    await this.flush();
  ...
  async readDeepRun(runId: string): Promise<OpenCodeTraceEventV1[]> {
    await this.flush();
    return this.readJsonl(path.join(this.rootDirectory, 'v1', 'deep', `${runId}.jsonl`))
      .catch(() => []);
  }

  async readRuntimeSegment(runtimeSegmentId: string): Promise<OpenCodeTraceEventV1[]> {
    await this.flush();
  ...
  async exportTraceBundle(traceId: string, targetDirectory: string): Promise<string> {
    ...
    const bundleDirectory = path.join(targetDirectory, `opencode-trace-${traceId}-${safeTimestamp}`);
      ...
      for (const record of batch) {
        ...
        if (record.deep && record.event.runId) {
          ...
          if (nextBytes <= MAX_DEEP_RUN_BYTES) {
            ...
            const cappedEvent: OpenCodeTraceEventV1 = {
              ...record.event,
              severity: 'warning',
  ...
  private appendMemory(batch: QueuedRecord[]): void {
  ...
  private updateSummary(event: OpenCodeTraceEventV1, deep: boolean): void {
  ...
  private async readJsonl(file: string): Promise<OpenCodeTraceEventV1[]> {
    const content = await fs.readFile(file, 'utf8');
    const events: OpenCodeTraceEventV1[] = [];
    for (const line of content.split('\n')) {
      if (!line.trim()) continue;
      try {
        events.push(JSON.parse(line) as OpenCodeTraceEventV1);
      } catch {
  ...
  private degrade(error: unknown, template?: OpenCodeTraceEventV1): void {
    if (this.mode === 'memory') return;
    this.mode = 'memory';
  ...
  private isRuntimeEvent(event: OpenCodeTraceEventV1): boolean {
    return !event.sessionId && event.traceId === event.runtimeSegmentId;
  }

  private higherSeverity(
    left: OpenCodeTraceSummary['highestUnreadSeverity'],
    right: OpenCodeTraceSummary['highestUnreadSeverity'],
  ): OpenCodeTraceSummary['highestUnreadSeverity'] {
    const severityOrder = ['debug', 'info', 'warning', 'critical', 'error'];
  ...
  private mergeEventCopies(
    diskEvents: OpenCodeTraceEventV1[],
    memoryEvents: OpenCodeTraceEventV1[],
  ): OpenCodeTraceEventV1[] {
    const events = new Map<string, OpenCodeTraceEventV1>();
