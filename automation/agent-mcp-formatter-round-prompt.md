...
8. If the implementation path uses background tasks, do not treat the main pass as complete until those background tasks finish, the repo-visible work they own has landed, and the final round artifacts required by this scaffold exist.
9. A clean main-process exit is not enough when background tasks were used. Before moving on, confirm there are no still-running background tasks tied to the implementation pass and that the round's final output contract has actually been written.
10. Only interrupt or retry an OpenCode implementation pass early when there is a hard failure signal: the wrapper exits non-zero, the log proves a concrete blocker, or the operator explicitly asks to stop. Lack of edits alone is not a blocker.
11. If the OpenCode wrapper exits with `124`, treat it as a timeout rather than an automatic round failure. Inspect the partial diff and the implementation log, tighten the brief if needed, and run another OpenCode pass unless a real blocker is proven.
